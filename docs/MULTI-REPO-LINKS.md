...
| Developer/Create pending-confirmation supplemental audit visual (`scenarioId=developer-create-drugi-vizuel-pending-title-and-citation-confirmation`) | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe summary metadata that a pending visual-reference slot exists; raw asset interpretation, unconfirmed title/citation details, and EXTREM/EXTRONDOL internals stay repo-local until human review confirms the canonical phrasing |
