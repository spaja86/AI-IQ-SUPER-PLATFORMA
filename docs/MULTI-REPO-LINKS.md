...
| Developer/Create `RADIO` bounded media/distribution/audio package | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe summary fields (`developerAndCreateRepoWideReflection.radioTrack`, `developerAndCreateRepoWideReflection.radioTrack.mikrofonProjectionAlias`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radioTrack`, `spajaKod.publicSignals.radioStatus`, `spajaKod.developerAndCreateImplementationPackage.radioSummary`) covering canonical alias, status, blocker/watch reasons, review posture, downstream reference, semantic-preservation snapshot, bounded radio summary and bounded `MIKROFON == MEGAFON, DISTRIBUTER, SAKSOFON` projection summary; keep raw token-model internals repo-local and do not create a downstream source-of-truth system |
| Developer/Create `MUZIČKA KUTIJA` bounded audio/instrument alias | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only summary-safe fields (`developerAndCreateRepoWideReflection.muzickaKutijaTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.muzickaKutijaTrack`, `spajaKod.publicSignals.muzickaKutijaStatus`, `spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary`) covering canonical alias, status, blocker/watch reasons, review posture, downstream reference, bounded `musicBoxSummary` and existing-layer mapping (`PARADIJOGONALNO PROGRAMIRANJE`, `METRIČKO PROGRAMIRANJE`, `SINEMETRIČKO PROGRAMIRANJE`, `AUDIO-VIZUELNI KONTRABAS PAKET`); keep raw audio/instrument internals, token models and governance formulas repo-local |
...
| Developer/Create V700 extension + `RANDOM selekcija svega` posture | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only summary-safe governance fields (`currentImplementationStage.v700Extension`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.v700Extension`, `developerAndCreateRepoWideReflection.roadmapExecution.v700Extension`, `spajaKod.developerAndCreateImplementationPackage.napoleonDiskaveriSummary.randomSelectionPosture`) covering rollout/rollback/human-review/downstream reference and bounded random-selection status; keep raw internals repo-local and keep active execution stage unchanged |
...
| Developer/Create `LEKSIKON` bounded interpretativni alias | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only summary-safe fields (`developerAndCreateRepoWideReflection.leksikonTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.leksikonTrack`, `spajaKod.publicSignals.leksikonStatus`, `spajaKod.developerAndCreateImplementationPackage.leksikonSummary`) covering status, blocker/watch reasons, review posture, downstream reference and bounded lexicon summary; keep raw internals/formule/token model repo-local and preserve lock (`DOK/DIK/FOR=EXTREM`, `DAK/DUK=EXTRONDOL`, `SPAJA KOD=summary-only`) |
| Developer/Create `PROMOCIJE / TIKETI / BONUSI / PROPUSNICE / ADMINISTRATIVNI BONUSI` bounded enterprise/governance package | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe summary fields (`developerAndCreateRepoWideReflection.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack`, `spajaKod.publicSignals.promocijeTiketiBonusiPropusniceAdministrativniBonusiStatus`, `spajaKod.developerAndCreateImplementationPackage.promocijeTiketiBonusiPropusniceAdministrativniBonusiSummary`) covering canonical alias, status, blocker/watch reasons, review posture, downstream reference, bounded subtrack statuses and enterprise summary; keep raw EXTREM/EXTRONDOL formulas, internal ticket evidence and administrative internals repo-local |
| Developer/Create `KRALJEVSKI POKLONI ZA SVAČIJI ROĐENDAN` bounded package | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe summary fields (`developerAndCreateRepoWideReflection.kraljevskiPokloniZaSvacijiRodjendanTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiPokloniZaSvacijiRodjendanTrack`, `spajaKod.publicSignals.kraljevskiPokloniZaSvacijiRodjendanStatus`, `spajaKod.developerAndCreateImplementationPackage.kraljevskiPokloniZaSvacijiRodjendanSummary`) covering canonical alias, status, blocker/watch reasons, review posture, fallbackInputStatus, downstream reference and bounded birthday-gift summary; keep raw token interpretation, internal gift logic and EXTREM/EXTRONDOL formulas repo-local |
| Developer/Create `KRALJEVSKI RAD` bounded package | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe summary fields (`developerAndCreateRepoWideReflection.kraljevskiRadTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiRadTrack`, `spajaKod.publicSignals.kraljevskiRadStatus`, `spajaKod.developerAndCreateImplementationPackage.kraljevskiRadSummary`) covering canonical alias, status, blocker/watch reasons, review posture, fallbackInputStatus, downstream reference, tokenOrderStatus and duplicateRuleStatus for the bounded sequence `DIR,DUR,DAR,RER,DIK,DUR,DAR,DJOMPA,DOKAT,KRUNA,ZOMBAT,DUKUS,NIKSON,KITAN,DIKAT,KVATRO,KALIMERO`; keep raw token interpretation and EXTREM/EXTRONDOL formulas repo-local |
...
| Developer/Create `IZVEŠTAJ` audit snapshot | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe branch-report fields (`developerAndCreateRepoWideReflection.branchReport`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.implementationPackage`, `spajaKod.developerAndCreateImplementationPackage.branchReport`) covering canonical alias `DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == IZVEŠTAJ`, `branchCompletionPercent`, `platformCompletionPercent`, `promotionReadinessStatus`, `nextStep`, `rolloutPlan`, `rollbackPlan`, `humanReviewStatus`, `downstreamReference`, measured branch layers (`docs/types/routes/tests/workflows`) and four-track summary (`technical/governance/public-boundary/business`); keep raw EXTREM/EXTRONDOL formulas and internal governance evidence repo-local |
...
| Developer/Create KRALJEVSTVO / ČOVEČANSTVO political campaign supplemental audit record (`scenarioId=kraljevstvo-covecanstvo-politicka-kompanja-ekstremno-maksimalno-profesionalna-audit-safe-developer-create`) | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only audit-safe supplemental visual-summary fields (`developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences`, `spajaKod.developerAndCreateVisualReflection.supplementalVisualReferences`) and keep raw interpretation repo-local; linked repo may consume only summary metadata for bounded vocabulary lock `EXTRIMLI EXTRONDOL EXTREM DOK DUK DAK DIK FOR`, ownership split (`DOK+DIK+FOR=EXTREM`, `DAK+DUK=EXTRONDOL`, `SPAJA KOD=summary-only`), and audit-safe governance posture (`human-review`, `WAWE`, `promotion/freeze`, `rollout/rollback`, `releaseAuditSummary`) |
...
| Developer/Create KRALJEVSKI RAD template supplemental audit visual (`scenarioId=kraljevski-rad-digitalna-industrija-znanje-tehnologija-ljudi-priroda-buducnost-developer-create`) | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only summary-only audit-safe fields (`scenarioId`, readiness status, review posture, ownership lock summary, bounded thematic labels) from (`developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences`, `spajaKod.developerAndCreateVisualReflection.supplementalVisualReferences`); linked repo may consume only summary metadata for kraljevski-rad/digitalna-industrija/kompanija-spaja/znanje/tehnologija/ljudi/priroda/buducnost themes while raw image interpretation remains repo-local |
| Developer/Create repo-wide RADNI TAKT supplemental audit visual (`scenarioId=developer-create-vrh-radni-takt-da-se-odrazi-na-svemu-u-repozitorijumu-vuk-developer-create`) | `spaja86/IO-OPENUI-AO` — Follow-up required | Sync only summary-only audit-safe fields (`scenarioId`, readiness status, blocker/watch reason, review posture, ownership lock summary, bounded thematic labels) from (`developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences`, `spajaKod.developerAndCreateVisualReflection.supplementalVisualReferences`); keep raw visual interpretation and internal governance formulas repo-local and preserve bounded vocabulary lock `EXTRIMLI EXTRONDOL EXTREM DOK DUK DAK DIK FOR` |
...
- Linked repo: `spaja86/IO-OPENUI-AO` — Follow-up only / audit-safe summary sync.
- Sync only audit-safe outputs (`developerAndCreateRepoWideReflection.kraljevskiPokloniZaSvacijiRodjendanTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiPokloniZaSvacijiRodjendanTrack`, `spajaKod.publicSignals.kraljevskiPokloniZaSvacijiRodjendanStatus`, `spajaKod.developerAndCreateImplementationPackage.kraljevskiPokloniZaSvacijiRodjendanSummary`) covering status, blocker/watch posture, review posture, fallbackInputStatus, downstream reference i bounded birthdayGiftSummary.
- Raw bounded-token interpretation internals and raw EXTREM/EXTRONDOL formulas remain repo-local.
- Ownership lock remains unchanged: `DOK/DIK/FOR -> EXTREM`, `DAK/DUK -> EXTRONDOL`, `SPAJA KOD -> summary-only`.

## Developer/Create KRALJEVSKI RAD downstream reference

- Scope lock: `DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KRALJEVSKI RAD`.
- Locked bounded token sequence: `DIR, DUR, DAR, RER, DIK, DUR, DAR, DJOMPA, DOKAT, KRUNA, ZOMBAT, DUKUS, NIKSON, KITAN, DIKAT, KVATRO, KALIMERO`.
- Validation lock: strict order required + duplicate rule (`DUR=2`, `DAR=2`) + deterministic fallback for `NaN`, `Infinity`, `empty`, `conflict`.
- Linked repo: `spaja86/IO-OPENUI-AO` — Follow-up only / audit-safe summary sync.
- Sync only audit-safe outputs (`developerAndCreateRepoWideReflection.kraljevskiRadTrack`, `releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiRadTrack`, `spajaKod.publicSignals.kraljevskiRadStatus`, `spajaKod.developerAndCreateImplementationPackage.kraljevskiRadSummary`) covering status, blocker/watch posture, review posture, downstream reference, tokenOrderStatus, duplicateRuleStatus and fallbackInputStatus.
- Raw bounded-token interpretation internals and raw EXTREM/EXTRONDOL formulas remain repo-local.
- Ownership lock remains unchanged: `DOK/DIK/FOR -> EXTREM`, `DAK/DUK -> EXTRONDOL`, `SPAJA KOD -> summary-only`.
...
- Linked repo: `spaja86/IO-OPENUI-AO` — Follow-up only / audit-safe summary sync.
- Sync only audit-safe tracker-summary outputs covering `canonicalAlias`, `status`, `conflictIntensity`, `currentWave`, `auditEvidence`, `rollbackReadiness`, `reviewPosture`, `promotionFreeze` i `downstreamReference` (follow-up only dok runtime contract surface ne uvede eksplicitna imena polja).
