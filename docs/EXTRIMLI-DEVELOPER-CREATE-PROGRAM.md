...
- U okviru tog istog zaključanog modela, `POLJOPRIVREDNI FAKULTET`, `GRAĐEVINSKI FAKULTET`, `PEDAGOŠKI FAKULTET` i `PSIHOLOŠKI FAKULTET` postaju additive-only oblasti u katalogu `testovi po oblastima`: prvi pokriva obrazovno-sertifikacioni ulaz za poljoprivrednike i gostoprimstvo, drugi projektovanje, infrastrukturu i operativnu spremnost gazdinstva, treći obrazovni/mentorski/metodološki i komunikacioni readiness, a četvrti bounded cognitive/readiness/resilience track bez kliničkog, dijagnostičkog ili terapijskog subsistema.
