...
- `PROGRAM_LOCK_ROLE_SPLIT`: `PROPORCIONALNO PROGRAMIRANJE` ostaje parent disciplina, `METRIČKO PROGRAMIRANJE` nosi satnicu i deklarativno-instancijsku metriku, `SINEMETRIČKO PROGRAMIRANJE` nosi vokalni/narativni audit-safe sloj, a `PARADIJOGONALNO PROGRIMIRANJE` iz `docs/EXTRIMLI-PARADIJOGONALNO-PROGRIMIRANJE.md` ostaje instrument tabla za pregled i signalnu kontrolu.
