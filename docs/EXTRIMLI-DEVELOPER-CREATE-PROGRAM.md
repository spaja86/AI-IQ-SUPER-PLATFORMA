...
- Kanonski governance rečnik ostaje zaključan: `EXTRIMLI EXTRONDOL EXTREM`, `DOK DIK DAK DUK FOR`, `KRALJEVSKI PRAVNI UNIVERZITET`, `KRALJEVSKI EKONOMSKI UNEVERZITET` sa istim ownership split-om (`EXTREM` tehnički signal, `EXTRONDOL` WAWE/audit governance, `SPAJA KOD` audit-safe summary).
