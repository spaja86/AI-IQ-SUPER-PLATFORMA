...
- `ČOVEČNOST` asset (`scenarioId=covecnost-developer-create-vrh-radni-takt`) ostaje primarni audit-only vizuelni odraz repo-wide discipline, a `ČOVEČANSTVO — ŽIVOT JE NAJVEĆA IGRA` asset (`scenarioId=covecanstvo-poker-zivotna-igra-developer-create`) ostaje supplemental audit/reference dokaz: EXTREM objavljuje bounded readiness/conflict signal, EXTRONDOL ga koristi samo za WAWE/review/rollback evidence, a SPAJA KOD izlaže samo javni audit-safe summary bez sirovih formula.
