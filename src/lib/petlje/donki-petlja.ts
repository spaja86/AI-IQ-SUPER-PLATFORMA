...
const GOAL = 'Težinsko praćenje pređenog koraka ka targetu kroz svako sledeće prizemljenje.';

export function runDonkiPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DONKI PETLJA', GOAL, normalized);
  ...
  while (current !== normalized.target) {
    ...
    output += Math.abs(current - previous) * guard.getIterations();
