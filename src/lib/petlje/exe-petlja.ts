...
const GOAL = 'Težinsko izvršavanje sekvence gde svaki element doprinosi po svom indeksu.';

export function runExePetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('EXE PETLJA', GOAL, normalized);
