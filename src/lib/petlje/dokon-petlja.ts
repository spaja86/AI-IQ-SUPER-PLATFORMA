...
const GOAL = 'Sabiranje svih međupozicija koje petlja posećuje dok zatvara put ka targetu.';

export function runDokonPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOKON PETLJA', GOAL, normalized);
