...
const GOAL = 'Brojanje posećenih vrednosti u opsegu koje dosežu ili prelaze target prag.';

export function runDombarPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOMBAR PETLJA', GOAL, normalized);
  ...
  while ((ascending && current <= normalized.end) || (!ascending && current >= normalized.end)) {
    ...
    guard.tick();
    if (current >= normalized.target) output += 1;
