...
const GOAL = 'Sabiranje apsolutnog odstupanja posećenih vrednosti od centralne ose opsega.';

export function runDombrePetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOMBRE PETLJA', GOAL, normalized);
  ...
  const trace = [];
  const midpoint = (normalized.start + normalized.end) / 2;
  let output = 0;
  ...
  while ((ascending && current <= normalized.end) || (!ascending && current >= normalized.end)) {
    ...
    guard.tick();
    output += Math.abs(current - midpoint);
