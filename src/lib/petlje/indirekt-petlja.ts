...
const GOAL = 'Indirektno približavanje targetu kroz waypoint sekvencu uz sabiranje svakog poboljšanja distance od početne pozicije.';

export function runIndirektPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('INDIREKT PETLJA', GOAL, normalized);
  ...
  const trace = [];
  let output = 0;
  let anchorDistance = Math.abs(normalized.target - normalized.start);
  ...
  for (const value of normalized.sequence) {
    ...
    if (distance < anchorDistance) {
      output += anchorDistance - distance;
