...
const GOAL = 'Direktno zatvaranje distance ka targetu bez skretanja, uz sabiranje ostvarenog pomaka po koraku.';

export function runDirektPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DIREKT PETLJA', GOAL, normalized);
  ...
  while (current !== normalized.target) {
    ...
    guard.tick();
    const previousDistance = Math.abs(normalized.target - current);
    current = current < normalized.target
      ? Math.min(current + step, normalized.target)
      : Math.max(current - step, normalized.target);
    const nextDistance = Math.abs(normalized.target - current);
    output += previousDistance - nextDistance;
