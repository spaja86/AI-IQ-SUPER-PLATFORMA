...
const GOAL = 'Težinsko sabiranje apsolutnog odstupanja sekvence od targeta.';

export function runZumbaPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('ZUMBA PETLJA', GOAL, normalized);
  ...
  for (const value of normalized.sequence) {
    ...
    guard.tick();
    output += Math.abs(value - normalized.target) * guard.getIterations();
