...
const GOAL = 'Brojanje potrebnih pomaka do targeta uz bounded target izvršavanje.';

export function runOmbaPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('OMBA PETLJA', GOAL, normalized);
