...
const GOAL = 'Težinsko sabiranje signed odstupanja od target centra po redosledu obilaska opsega.';

export function runDombraPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOMBRA PETLJA', GOAL, normalized);
  ...
  while ((ascending && current <= normalized.end) || (!ascending && current >= normalized.end)) {
    ...
    guard.tick();
    output += (current - normalized.target) * guard.getIterations();
