...
const GOAL = 'Sabiranje potpisanog odstupanja svake posećene vrednosti u opsegu u odnosu na target centar.';

export function runSarPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('SAR PETLJA', GOAL, normalized);
  ...
  while ((ascending && current <= normalized.end) || (!ascending && current >= normalized.end)) {
    ...
    guard.tick();
    output += current - normalized.target;
