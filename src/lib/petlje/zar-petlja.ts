...
const GOAL = 'Sabiranje apsolutnih razlika između susednih elemenata sekvence.';

export function runZarPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('ZAR PETLJA', GOAL, normalized);
  ...
  for (let index = 1; index < normalized.sequence.length; index++) {
    if (!enteredMonster) {
    ...
    guard.tick();
    const delta = Math.abs(normalized.sequence[index] - normalized.sequence[index - 1]);
    output += delta;
    trace.push({ iteration: guard.getIterations(), value: delta, accumulator: output });
  }
