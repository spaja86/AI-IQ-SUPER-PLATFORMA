...
const GOAL = 'Brojanje posećenih vrednosti u opsegu koje su manje ili jednake targetu.';

export function runUkPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('UK PETLJA', GOAL, normalized);
