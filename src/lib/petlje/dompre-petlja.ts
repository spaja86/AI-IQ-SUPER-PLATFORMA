...
const GOAL = 'Akumulacija preostale distance do targeta pre svakog pomeraja.';

export function runDomprePetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOMPRE PETLJA', GOAL, normalized);
