...
const GOAL = 'Praćenje sekvence kroz najbolju dosadašnju bliskost targetu i sabiranje svakog poboljšanja.';

export function runDikPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DIK PETLJA', GOAL, normalized);
