...
const GOAL = 'Sabiranje kubova svih posećenih vrednosti u opsegu uz deterministički audit trag.';

export function runDjuprePetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DJUPRE PETLJA', GOAL, normalized);
