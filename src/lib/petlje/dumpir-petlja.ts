...
const GOAL = 'Brojanje elemenata sekvence koji ostaju na ili ispod target praga.';

export function runDumpirPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DUMPIR PETLJA', GOAL, normalized);
