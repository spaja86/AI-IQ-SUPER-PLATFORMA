...
const GOAL = 'Pronalaženje prvog indeksa u sekvenci koji tačno odgovara target vrednosti.';

export function runIziPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('IZI PETLJA', GOAL, normalized);
  ...
  for (const [index, value] of normalized.sequence.entries()) {
    if (!enteredMonster) {
    ...
    guard.tick();
    if (output === -1 && value === normalized.target) output = index;
