...
const GOAL = 'Koračno približavanje targetu uz akumulaciju preostale distance posle svakog pomeraja.';

export function runDokPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOK PETLJA', GOAL, normalized);
  ...
  while (current !== normalized.target) {
    ...
    output += Math.abs(normalized.target - current);
