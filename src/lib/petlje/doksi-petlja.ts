...
const GOAL = 'Sabiranje signed pomaka između uzastopnih elemenata sekvence.';

export function runDoksiPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOKSI PETLJA', GOAL, normalized);
