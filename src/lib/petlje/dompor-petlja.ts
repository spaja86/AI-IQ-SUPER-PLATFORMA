...
const GOAL = 'Sabiranje razlike između maksimuma opsega i svake posećene vrednosti.';

export function runDomporPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOMPOR PETLJA', GOAL, normalized);
  ...
  const trace = [];
  const rangeMax = Math.max(normalized.start, normalized.end);
  let output = 0;
