...
const GOAL = 'Sabiranje apsolutnog odstupanja svake posećene vrednosti od target vrednosti.';

export function runDorPetlja(input: PetljaInput): PetljaResult {
  const normalized = normalizeInput(input);
  const result = baseResult('DOR PETLJA', GOAL, normalized);
