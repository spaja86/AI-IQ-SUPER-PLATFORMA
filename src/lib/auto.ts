...
function computeAutonomijaScore(): number {
  return clampScore(
    62
    + Math.min(24, TOTAL_PAGES / AUTONOMIJA_PAGES_NORMALIZATION_DIVISOR * 10)
    + Math.min(14, TOTAL_DIAGNOSTIKA / 240),
  );
}
...
function computeUpravljanjeScore(): number {
  return clampScore(
    60
    + Math.min(22, TOTAL_PROTOKOLA * 1.2)
    + Math.min(18, OMEGA_AI_PERSONA_COUNT / 2),
  );
}
...
function computeTransformacijaScore(): number {
  return clampScore(
    58
    + Math.min(22, OMEGA_AI_PERSONA_COUNT)
    + Math.min(14, OMEGA_AI_OKTAVA_COUNT),
  );
}
...
function computeOrkestracijaScore(): number {
  return clampScore(
    55
    + Math.min(21, TOTAL_API_ROUTES / ORKESTRACIJA_API_ROUTE_NORMALIZATION_DIVISOR * 1.5)
    + Math.min(24, TOTAL_ROUTES / 80),
  );
}
...
function computeOptimizacijaScore(): number {
  return clampScore(
    57
    + Math.min(25, TOTAL_IGRICA / 5)
    + Math.min(18, TOTAL_DIAGNOSTIKA / 200),
  );
}
...
function computeAutomatizacijaScore(): number {
  return clampScore(
    52
    + Math.min(24, AUTOFINISH_COUNT / AUTOMATIZACIJA_AUTOFINISH_NORMALIZATION_DIVISOR)
    + Math.min(24, TOTAL_API_ROUTES / ORKESTRACIJA_API_ROUTE_NORMALIZATION_DIVISOR),
  );
}
