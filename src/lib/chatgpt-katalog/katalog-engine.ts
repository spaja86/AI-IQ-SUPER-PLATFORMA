...
import {
  ...
  CHATGPT_KATALOG_LINKED_MODULES,
  CHATGPT_KATALOG_LINKED_REPOS,
...
export function getKatalogHealth(): KatalogHealth {
  const activeModelCount = GPT_MODELS.filter((model) => model.status === 'active').length;
  return {
    status: 'ok',
    ...
    linkedModules: [...CHATGPT_KATALOG_LINKED_MODULES],
    linkedRepos: [...CHATGPT_KATALOG_LINKED_REPOS],
