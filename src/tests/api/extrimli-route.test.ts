...
async function runTests(): Promise<void> {
  ...
  await test('GET /api/extrimli/spaja-kod returns encapsulated facade and headers', async () => {
    ...
    assert(branchReport.scopeStatement.includes('additive-only bounded branch report / audit snapshot alias'), 'unexpected SPAJA KOD branch report scope statement');
    assert(branchReport.reportLock.sourceOfTruth === '/api/extrimli/extrondol', 'unexpected SPAJA KOD branch report lock source');
    assert(branchReport.reportLock.canonicalAlias === branchReport.canonicalAlias, 'unexpected SPAJA KOD branch report lock canonical alias');
    assert(branchReport.reportLock.canonicalFormat === branchReport.canonicalFormat, 'unexpected SPAJA KOD branch report lock canonical format');
    assert(branchReport.reportLock.roadmapStageId === 'v5-extrondol-release-audit-and-orchestration', 'unexpected SPAJA KOD branch report lock roadmap stage');
    assert(branchReport.reportLock.downstreamReference === 'docs/MULTI-REPO-LINKS.md -> spaja86/IO-OPENUI-AO (summary-only)', 'unexpected SPAJA KOD branch report lock downstream reference');
    assert(branchReport.reportLock.requiredAuditBlocks.join(',') === 'completed,partial,blocked,branchCompletionPercent,platformCompletionPercent,promotionReadinessStatus,nextStep,rolloutPlan,rollbackPlan,humanReviewStatus,downstreamReference,measurableOutput,acceptanceEvidence', 'unexpected SPAJA KOD branch report required audit blocks');
    ...
    assert(branchReport.reportLock.ownershipSplit.dokDikFor === 'EXTREM', 'unexpected SPAJA KOD branch report DOK/DIK/FOR ownership');
    assert(branchReport.reportLock.ownershipSplit.dakDuk === 'EXTRONDOL', 'unexpected SPAJA KOD branch report DAK/DUK ownership');
    assert(branchReport.reportLock.ownershipSplit.spajaKod === 'audit-safe-summary-only', 'unexpected SPAJA KOD branch report SPAJA KOD ownership');
    ...
    assert(body.data.developerAndCreateImplementationPackage.vinogradiGrockaRestoranSummary.status === body.data.publicSignals.vinogradiGrockaRestoranStatus, 'unexpected SPAJA KOD VINOGRADI GROCKA, RESTORAN summary/status mismatch');
    ...
    const spajaKodPolitickaKompanjaKraljevstvoSupplemental = body.data.developerAndCreateVisualReflection.supplementalVisualReferences.find((reference: { canonicalNarrativeId: string; thematicSignals: string[]; imageToSignalProfile: { scenarioId: string; ownershipLock: { spajaKod: string } } }) => reference.canonicalNarrativeId === 'kraljevstvo-covecanstvo-kralj-nad-kraljevima-spajic-nikola-politicka-kompanja-ekstremno-maksimalno-profesionalna-developer-create');
    assert(spajaKodPolitickaKompanjaKraljevstvoSupplemental?.visualReference === 'documentation-only://kraljevstvo-covecanstvo-kralj-nad-kraljevima-spajic-nikola-politicka-kompanja-ekstremno-maksimalno-profesionalna', 'unexpected SPAJA KOD developer/create KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental visual reference');
    assert(spajaKodPolitickaKompanjaKraljevstvoSupplemental?.imageToSignalProfile.scenarioId === 'kraljevstvo-covecanstvo-politicka-kompanja-ekstremno-maksimalno-profesionalna-audit-safe-developer-create', 'unexpected SPAJA KOD developer/create KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental scenario id');
    assert(spajaKodPolitickaKompanjaKraljevstvoSupplemental?.thematicSignals.join(',') === 'kraljevstvo-covecanstvo,kralj-nad-kraljevima,spajic-nikola-public-presentation,politicka-kompanja-ekstremno-maksimalno-profesionalna,dok-dik-for-extrem,dak-duk-extrondol,spaja-kod-summary-only,audit-safe-release-governance', 'unexpected SPAJA KOD developer/create KRALJEVSTVO/ČOVEČANSTVO political campaign thematic signals');
    assert(spajaKodPolitickaKompanjaKraljevstvoSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'unexpected SPAJA KOD developer/create KRALJEVSTVO/ČOVEČANSTVO political campaign boundary');
