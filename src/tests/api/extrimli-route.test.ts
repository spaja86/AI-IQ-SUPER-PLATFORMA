...
async function runTests(): Promise<void> {
  ...
  await test('GET /api/extrimli/extrondol returns orchestration report and headers', async () => {
    ...
    const responsePendingSecondVisualSupplemental = body.data.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference: { canonicalNarrativeId: string; thematicSignals: string[]; citation: string; imageToSignalProfile: { scenarioId: string; ownershipLock: { dokDikFor: string; dakDuk: string; spajaKod: string } } }) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(responsePendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'unexpected developer/create pending second visual supplemental visual reference');
    assert(responsePendingSecondVisualSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-drugi-vizuel-pending-title-and-citation-confirmation', 'unexpected developer/create pending second visual supplemental scenario id');
    assert(responsePendingSecondVisualSupplemental?.citation.includes('dok naziv i citat ne budu ljudski potvrđeni'), 'unexpected developer/create pending second visual citation');
    assert(responsePendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'unexpected developer/create pending second visual thematic signals');
    assert(responsePendingSecondVisualSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'unexpected developer/create pending second visual EXTREM ownership');
    assert(responsePendingSecondVisualSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'unexpected developer/create pending second visual EXTRONDOL ownership');
    assert(responsePendingSecondVisualSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'unexpected developer/create pending second visual SPAJA KOD boundary');
    ...
    const releaseAuditPendingSecondVisualSupplemental = body.data.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences.find((reference: { canonicalNarrativeId: string; thematicSignals: string[]; citation: string; imageToSignalProfile: { scenarioId: string; signalOutputs: { readinessStatus: string } } }) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(releaseAuditPendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'unexpected release-audit developer/create pending second visual supplemental visual reference');
    assert(releaseAuditPendingSecondVisualSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-drugi-vizuel-pending-title-and-citation-confirmation', 'unexpected release-audit developer/create pending second visual supplemental scenario id');
    assert(releaseAuditPendingSecondVisualSupplemental?.citation.includes('dok naziv i citat ne budu ljudski potvrđeni'), 'unexpected release-audit developer/create pending second visual citation');
    assert(releaseAuditPendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'unexpected release-audit developer/create pending second visual thematic signals');
    assert(releaseAuditPendingSecondVisualSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === body.data.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.status, 'unexpected release-audit developer/create pending second visual readiness status');
  ...
  await test('GET /api/extrimli/spaja-kod returns encapsulated facade and headers', async () => {
    ...
    assert(spajaKodKraljevstvoZvanicnoPravoLiceSupplemental?.visualReference.includes('82f065a8-3bd7-4c09-8d9f-9c66b8443d66'), 'unexpected SPAJA KOD developer/create KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) supplemental visual reference');
    ...
    assert(spajaKodKraljevstvoZvanicnoPravoLiceSupplemental?.thematicSignals.join(',') === 'kraljevstvo,zajednistvo,znanje,priroda,tehnologija,humanost', 'unexpected SPAJA KOD developer/create KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) thematic signals');
    ...
    const spajaKodPendingSecondVisualSupplemental = body.data.developerAndCreateVisualReflection.supplementalVisualReferences.find((reference: { canonicalNarrativeId: string; thematicSignals: string[]; imageToSignalProfile: { signalOutputs: { readinessStatus: string } } }) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(spajaKodPendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'unexpected SPAJA KOD developer/create pending second visual supplemental visual reference');
    assert(spajaKodPendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'unexpected SPAJA KOD developer/create pending second visual thematic signals');
    assert(spajaKodPendingSecondVisualSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === body.data.publicSignals.developerAndCreateStatus, 'unexpected SPAJA KOD developer/create pending second visual readiness status');
