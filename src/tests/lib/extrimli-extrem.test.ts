...
async function runTests(): Promise<void> {
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health preserves EXTREM vs EXTRONDOL ownership boundary', () => {
    ...
    assert(developerCreateKraljevstvoZvanicnoPravoLiceSupplemental?.visualReference.includes('82f065a8-3bd7-4c09-8d9f-9c66b8443d66'), 'developer/create KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) supplemental visual reference mismatch');
    assert(developerCreateKraljevstvoZvanicnoPravoLiceSupplemental?.thematicSignals.join(',') === 'kraljevstvo,zajednistvo,znanje,priroda,tehnologija,humanost', 'developer/create KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) thematic signals mismatch');
    ...
    const developerCreatePendingSecondVisualSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(Boolean(developerCreatePendingSecondVisualSupplemental), 'developer/create pending second visual supplemental narrative id mismatch');
    assert(developerCreatePendingSecondVisualSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-drugi-vizuel-pending-title-and-citation-confirmation', 'developer/create pending second visual supplemental scenario id mismatch');
    assert(developerCreatePendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'developer/create pending second visual supplemental visual reference mismatch');
    assert(developerCreatePendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'developer/create pending second visual thematic signals mismatch');
    assert(developerCreatePendingSecondVisualSupplemental?.citation.includes('dok naziv i citat ne budu ljudski potvrđeni'), 'developer/create pending second visual citation mismatch');
    assert(developerCreatePendingSecondVisualSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create pending second visual supplemental readiness status mismatch');
