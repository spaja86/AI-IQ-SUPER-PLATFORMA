...
async function runTests(): Promise<void> {
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health preserves EXTREM vs EXTRONDOL ownership boundary', () => {
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.roadmapStageId === 'v4-extrem-repo-wide-rhythm-readiness-expansion', 'developer/create EXTREM roadmap stage mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.acceptanceEvidence.join(',') === 'dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection,radniTaktMozgaMislilac.readiness,metrikoProgramiranje.readiness,sinemetrickoProgramiranje.readiness,paradijogonalnoProgrimiranje.readiness,vrhProgramskogEkviladenta.readiness', 'developer/create EXTREM acceptance evidence mismatch');
