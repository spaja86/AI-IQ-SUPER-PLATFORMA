...
async function runTests(): Promise<void> {
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health preserves EXTREM vs EXTRONDOL ownership boundary', () => {
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.justicePath.evidentiaryCompletenessStatus === 'BLOCKED', 'developer/create INSPEKTORI evidentiary completeness should drive the default review escalation');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.justicePath.reviewPosture === 'REVIEW_REQUIRED', 'developer/create INSPEKTORI review posture should escalate when justice-path evidence is blocked');
    assert(
