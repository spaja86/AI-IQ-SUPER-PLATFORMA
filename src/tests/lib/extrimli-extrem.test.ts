...
async function runTests(): Promise<void> {
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health preserves EXTREM vs EXTRONDOL ownership boundary', () => {
    ...
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.requiredGovernanceGates.includes('anti-abuse-review'), 'developer/create equipment sub-track anti-abuse gate mismatch');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.forbiddenEvidence.includes('tactical-instructions'), 'developer/create equipment sub-track must forbid tactical instructions');
