...
async function runTests(): Promise<void> {
  ...
  await test('default report keeps VINOGRADI GROCKA RESTORAN track removed from repo-wide reflection', () => {
    ...
    assert(implementationPackage.kraljevskiProgramskiUneverzitetBoundary.boundedFacultyDomains.join(',') === 'POLJOPRIVREDNI FAKULTET,GRAĐEVINSKI FAKULTET,MATEMATIČKI FAKULTET', 'developer/create implementation package bounded faculty domains mismatch');
    assert(implementationPackage.inspektoriBoundary.trackRole === 'justice-path-inspection-track', 'developer/create implementation package INSPEKTORI boundary role mismatch');
    assert(implementationPackage.inspektoriBoundary.noOperationalSecurityDetails, 'developer/create implementation package INSPEKTORI boundary must forbid operational details');
    ...
    assert(implementationPackage.kraljevskiDrustveniPoredakBoundary.linkedDomains.join(',') === 'GRAĐEVINSKI AKT,KRALJEVSKA DOPUNA,KRALJEVSKA VOJNA I POLICIJSKA OPREMA', 'developer/create implementation package social-order linked domains mismatch');
    ...
    assert(['READY', 'WATCH', 'BLOCKED'].includes(implementationPackage.smartProgramskiJezikPackage.technicalProfile.immersiveVisualizationStatus), 'developer/create smart language immersive status mismatch');
    assert(implementationPackage.smartProgramskiJezikPackage.technicalProfile.dimensionalProgression.join(',') === '360D,720D,1440D,2880D,5760D', 'developer/create smart language dimensional progression mismatch');
    ...
    assert(implementationPackage.eksperimentProgramskiJezikBoundary.immersiveScenesGovernedBy === 'developerAndCreateRepoWideReflection.technicalReadinessProfile.immersiveVisualization3dTrack', 'developer/create implementation package Eksperiment Programski Jezik immersive binding mismatch');
    ...
    assert(radioTrack.mikrofonProjectionAlias.canonicalEquality === 'MIKROFON == MEGAFON, DISTRIBUTER, SAKSOFON', 'developer/create RADIO mikrofon projection equality mismatch');
    assert(radioTrack.mikrofonProjectionAlias.roleClassification === 'additive-only-bounded-mikrofon-projection-alias', 'developer/create RADIO mikrofon projection role classification mismatch');
    assert(radioTrack.mikrofonProjectionAlias.boundedTokenVocabulary.canonicalSequence.join(',') === 'MIKROFON,MEGAFON,DISTRIBUTER,SAKSOFON', 'developer/create RADIO mikrofon projection token sequence mismatch');
    assert(radioTrack.mikrofonProjectionAlias.semanticPreservation.megafonMeaning === 'bounded-public-amplification-projection-without-new-distribution-engine', 'developer/create RADIO mikrofon projection megafon semantic preservation mismatch');
    assert(radioTrack.mikrofonProjectionAlias.summarySafePublicFields.join(',') === 'canonicalEquality,status,blockerReason,watchReasons,reviewPosture,downstreamReference,mikrofonSummary,semanticPreservation', 'developer/create RADIO mikrofon projection summary-safe fields mismatch');
    ...
    assert(audioVisualKontrabasPackage.canonicalName === 'DEVELOPER AND CREATE / AUDIO-VIZUELNI KONTRABAS PAKET', 'developer/create audio-visual package canonical name mismatch');
    ...
    assert(sarkazamPrivrednaGranaDigitalizmaTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == SARKAZAM / PRIVREDNA GRANA DIGITALIZMA / PROJEKTI ENTUZIJAZMA PO ČINU OBLASTIMA', 'developer/create Sarkazam canonical alias mismatch');
    ...
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == PROMOCIJE / TIKETI / BONUSI / PROPUSNICE / ADMINISTRATIVNI BONUSI', 'developer/create promotions package canonical alias mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.roleClassification === 'additive-only-bounded-enterprise-promotions-tickets-bonuses-passes-admin-overrides-track', 'developer/create promotions package role classification mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.scopeStatement === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == PROMOCIJE / TIKETI / BONUSI / PROPUSNICE / ADMINISTRATIVNI BONUSI == additive-only bounded enterprise/governance paket pod Kompanija SPAJA / Digitalna Industrija bez novih runtime ruta i bez paralelnog source-of-truth sistema', 'developer/create promotions package scope statement mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.boundedSignals.join(',') === 'promocije-readiness,tiketi-evidence,bonusi-approval,propusnice-eligibility,administrativni-bonusi-human-review-override,conflict-signal,fallback-signal', 'developer/create promotions package bounded signals mismatch');
    assert(Object.values(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.boundedSubtracks).join(',') === 'promocije,tiketi,bonusi,propusnice,administrativni bonusi', 'developer/create promotions package subtracks mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.summarySafePublicFields.join(',') === 'canonicalAlias,status,blockerReason,watchReasons,reviewPosture,downstreamReference,enterpriseSummary,subtrackStatuses,humanReviewOverrideStatus', 'developer/create promotions package summary-safe fields mismatch');
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health preserves EXTREM vs EXTRONDOL ownership boundary', () => {
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.stocarstvo === 'STOČARSTVO', 'developer/create canonical STOČARSTVO vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.vinogradarstvo === 'VINOGRADARSTVO', 'developer/create canonical VINOGRADARSTVO vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.poljoprivredniFakultet === 'POLJOPRIVREDNI FAKULTET', 'developer/create canonical POLJOPRIVREDNI FAKULTET vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.gradjevinskiFakultet === 'GRAĐEVINSKI FAKULTET', 'developer/create canonical GRAĐEVINSKI FAKULTET vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.matematickiFakultet === 'MATEMATIČKI FAKULTET', 'developer/create canonical MATEMATIČKI FAKULTET vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.pedagoskiFakultet === 'PEDAGOŠKI FAKULTET', 'developer/create canonical PEDAGOŠKI FAKULTET vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.psiholoskiFakultet === 'PSIHOLOŠKI FAKULTET', 'developer/create canonical PSIHOLOŠKI FAKULTET vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.kompanijaSpaja === 'KOMPANIJA SPAJA', 'developer/create canonical KOMPANIJA SPAJA vocabulary mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.canonicalGovernanceVocabulary.digitalnaIndustrija === 'DIGITALNA INDUSTRIJA', 'developer/create canonical DIGITALNA INDUSTRIJA vocabulary mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.mappedTracks.kompanijaSpajaDigitalnaIndustrija === 'Kompanija SPAJA / Digitalna Industrija', 'developer/create mapped enterprise track mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.priorityExecutionOrder.join(',') === 'terminology-and-scope-lock,extrem-technical-readiness-profile,extrondol-release-audit-and-governance-mirror,spaja-kod-public-safe-summary,kompanija-spaja-digitalna-industrija-business-mapping,drift-zero-validation', 'developer/create priority execution order mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.fourTrackProgramPackage.businessTrack.canonicalName === 'Kompanija SPAJA / Digitalna Industrija', 'developer/create business track canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.fourTrackProgramPackage.businessTrack.noNewFinancialRuntimeFormulas, 'developer/create business track must forbid new financial runtime formulas');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.fourTrackProgramPackage.publicBoundaryTrack.summaryOnly, 'developer/create public boundary track must remain summary-only');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.unifiedNarrative === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KRALJEVSKI PROGRAMSKI UNEVERZITET', 'developer/create economic track unified narrative mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.boundedPrivredniDomains.stocarstvo.canonicalName === 'STOČARSTVO', 'developer/create STOČARSTVO canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.boundedPrivredniDomains.vinogradarstvo.canonicalName === 'VINOGRADARSTVO', 'developer/create VINOGRADARSTVO canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.boundedPrivredniDomains.stocarstvo.governanceBinding.linkedTracks.join(',') === 'PRIVREDNI AKT,ZADRUGA', 'developer/create STOČARSTVO governance linkage mismatch');
    assert(['READY', 'WATCH', 'BLOCKED'].includes(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.boundedPrivredniDomains.vinogradarstvo.infrastructurePosture), 'developer/create VINOGRADARSTVO infrastructure posture mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.privredniPuteviInterpretation.noNewFinancialRuntimeFormulas, 'developer/create economic track must keep privredni putevi additive-only');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiEkonomskiUneverzitet.pravniPoredakPolicyGate.noStandaloneEngine, 'developer/create economic track must keep legal-economic order policy-gated');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.domainTestCatalog.areas.length === 10, 'developer/create domain test catalog size mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.poljoprivredniFakultet.canonicalName === 'POLJOPRIVREDNI FAKULTET', 'developer/create POLJOPRIVREDNI FAKULTET canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.gradjevinskiFakultet.canonicalName === 'GRAĐEVINSKI FAKULTET', 'developer/create GRAĐEVINSKI FAKULTET canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.poljoprivredniFakultet.governanceBinding.linkedEconomicDomains.join(',') === 'STOČARSTVO,VINOGRADARSTVO', 'developer/create POLJOPRIVREDNI FAKULTET linked domains mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.matematickiFakultet.canonicalName === 'MATEMATIČKI FAKULTET', 'developer/create MATEMATIČKI FAKULTET canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.matematickiFakultet.technicalReadinessBinding.parentInterpretativeTrack === 'KRALJEVSKI MATEMATIČKI UNIVERZITET', 'developer/create MATEMATIČKI FAKULTET parent interpretative track mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.matematickiFakultet.deterministicFallbackPolicy.fallbackInputs.join(',') === 'NaN,Infinity,empty,conflict', 'developer/create MATEMATIČKI FAKULTET deterministic fallback mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.pedagoskiFakultet.canonicalName === 'PEDAGOŠKI FAKULTET', 'developer/create PEDAGOŠKI FAKULTET canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.pedagoskiFakultet.governanceBinding.linkedProgrammaticDomains.join(',') === 'POLJOPRIVREDNI FAKULTET,GRAĐEVINSKI FAKULTET', 'developer/create PEDAGOŠKI FAKULTET linked domains mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.psiholoskiFakultet.canonicalName === 'PSIHOLOŠKI FAKULTET', 'developer/create PSIHOLOŠKI FAKULTET canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.psiholoskiFakultet.nonClinicalBoundary.noClinicalSubsystem, 'developer/create PSIHOLOŠKI FAKULTET clinical boundary mismatch');
    assert(['READY', 'WATCH', 'BLOCKED'].includes(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.kraljevskiProgramskiUneverzitet.boundedFacultyDomains.gradjevinskiFakultet.readiness.status), 'developer/create GRAĐEVINSKI FAKULTET readiness mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.rolloutPlan.length > 0, 'developer/create current stage rollout plan mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.rollbackPlan.length > 0, 'developer/create current stage rollback plan mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.humanReviewStatus === 'required-before-promotion', 'developer/create current stage human review status mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.downstreamReference === 'docs/MULTI-REPO-LINKS.md -> spaja86/IO-OPENUI-AO (summary-only)', 'developer/create current stage downstream reference mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.implementationPackage.programTracksSummary.businessCanonicalName === 'Kompanija SPAJA / Digitalna Industrija', 'developer/create implementation package business canonical name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.implementationPackage.businessLayerSummary.umbrellaModel === 'DIGITALNA INDUSTRIJA', 'developer/create implementation package business umbrella mismatch');
    ...
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.governanceCatalogOnly, 'developer/create equipment sub-track must remain governance-catalog-only');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.categoryCatalog.join(',') === 'KRALJEVSKI,VOJNI,POLICIJSKI', 'developer/create equipment sub-track category catalog mismatch');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.qualityCriteria.standard === 'najbolja-savremena-oprema-auditabilno-neoperativno', 'developer/create equipment quality standard mismatch');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.requiredGovernanceGates.join(',') === 'human-review,compliance-review,anti-abuse-review,audit-trail,rollback-plan,downstream-sync', 'developer/create equipment sub-track required governance gates mismatch');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.forbiddenEvidence.join(',') === 'tactical-instructions,sensitive-map,operational-identity,weaponization-details,bank-account-number,kyc-document,payroll-secret', 'developer/create equipment sub-track forbidden evidence mismatch');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.operationalBoundary.noOperationalSecurityInstructions, 'developer/create equipment sub-track must forbid operational instructions');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.operationalBoundary.noTacticalOrSensitiveSecurityData, 'developer/create equipment sub-track must forbid tactical/sensitive security data');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.operationalBoundary.auditSafeSummaryOnly, 'developer/create equipment sub-track must remain audit-safe summary only');
    assert(socialOrder.kraljevskiAktBezbednosti.kraljevskaVojnaIPolicijskaOprema.readiness.status === socialOrder.kraljevskiAktBezbednosti.readiness.status, 'developer/create equipment sub-track readiness must stay aligned with parent security-act readiness');
    assert(['NaN', 'Infinity', 'empty', 'conflict'].join(',') === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.implementationPackage.validationLock.deterministicFallbackInputs.join(','), 'developer/create deterministic fallback inputs mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.aiIdentityFinanceGovernance.bezpovratneSubvencijeGovernance.canonicalName === 'BEZPOVRATNE SUBVENCIJE', 'AI identity-finance bezpovratne subvencije name mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.aiIdentityFinanceGovernance.bezpovratneSubvencijeGovernance.governanceOnlyModel, 'AI identity-finance bezpovratne subvencije must stay governance-only');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.aiIdentityFinanceGovernance.bezpovratneSubvencijeGovernance.allowedEvidence.join(',') === 'approval-status,payment-verification-status,audit-evidence,payout-readiness', 'AI identity-finance bezpovratne subvencije allowed evidence mismatch');
    ...
    const developerCreateMapeUmaEpilogSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'covecanstvo-epilog-mape-uma-slike-znacenje-developer-create');
    assert(Boolean(developerCreateMapeUmaEpilogSupplemental), 'developer/create EPILOG (MAPE UMA) supplemental narrative id mismatch');
    assert(developerCreateMapeUmaEpilogSupplemental?.visualReference.includes('f857f0fd-c29d-4749-aecd-f42745646e69'), 'developer/create EPILOG (MAPE UMA) supplemental visual reference mismatch');
    assert(developerCreateMapeUmaEpilogSupplemental?.imageToSignalProfile.scenarioId === 'covecanstvo-epilog-mape-uma-slike-znacenje-developer-create', 'developer/create EPILOG (MAPE UMA) supplemental scenario id mismatch');
    assert(developerCreateMapeUmaEpilogSupplemental?.thematicSignals.join(',') === 'mape-uma,slike-plus-znacenje,ucenje,znanje,kreativnost,saradnja,odrzivost,mir,covecanstvo-epilog', 'developer/create EPILOG (MAPE UMA) thematic signals mismatch');
    assert(developerCreateMapeUmaEpilogSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create EPILOG (MAPE UMA) supplemental readiness status mismatch');
    ...
    const developerCreateKraljevskaKucaSmederevoSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevska-kuca-srbija-smederevo-epicentricna-tacka-planete-developer-create');
    assert(Boolean(developerCreateKraljevskaKucaSmederevoSupplemental), 'developer/create KRALJEVSKA KUĆA supplemental narrative id mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.imageToSignalProfile.scenarioId === 'kraljevska-kuca-srbija-smederevo-jedan-planet-jedan-narod-jedna-buducnost-developer-create', 'developer/create KRALJEVSKA KUĆA supplemental scenario id mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.visualReference.includes('7b6cfca4-d61f-4974-b06f-299be5626282'), 'developer/create KRALJEVSKA KUĆA supplemental visual reference mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.thematicSignals.join(',') === 'kraljevska-kuca,srbija-smederevo,epicentricna-tacka-planete,jedan-planet-jedan-narod-jedna-buducnost,legal-governance-epilog,bounded-non-enforcement', 'developer/create KRALJEVSKA KUĆA thematic signals mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create KRALJEVSKA KUĆA EXTREM ownership mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create KRALJEVSKA KUĆA EXTRONDOL ownership mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create KRALJEVSKA KUĆA SPAJA KOD boundary mismatch');
    assert(developerCreateKraljevskaKucaSmederevoSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create KRALJEVSKA KUĆA supplemental readiness status mismatch');
    const developerCreateKraljevskiPoduhvatVukoviSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'projekat-srbija-beli-vuk-crni-vuk-developer-create');
    assert(Boolean(developerCreateKraljevskiPoduhvatVukoviSupplemental), 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK supplemental narrative id mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.imageToSignalProfile.scenarioId === 'kraljevski-poduhvat-vukovi-projekat-srbija-beli-vuk-crni-vuk-developer-create', 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK supplemental scenario id mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.visualReference.includes('b8ba7d39-2f0b-4016-a60d-0cdd6ac41bcf'), 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK supplemental visual reference mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.thematicSignals.join(',') === 'kraljevski-poduhvat,projekat-srbija,vukovi,beli-vuk,crni-vuk,bounded-governance-symbolics', 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK thematic signals mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK EXTREM ownership mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK EXTRONDOL ownership mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK SPAJA KOD boundary mismatch');
    assert(developerCreateKraljevskiPoduhvatVukoviSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create PROJEKAT SRBIJA — BELI VUK CRNI VUK supplemental readiness status mismatch');
    const developerCreateRepoWideRadniTaktVukSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'developer-create-vrh-radni-takt-svemu-u-repozitorijumu-vuk-developer-create');
    assert(Boolean(developerCreateRepoWideRadniTaktVukSupplemental), 'developer/create repo-wide RADNI TAKT VUK supplemental narrative id mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-vrh-radni-takt-da-se-odrazi-na-svemu-u-repozitorijumu-vuk-developer-create', 'developer/create repo-wide RADNI TAKT VUK supplemental scenario id mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.visualReference.includes('de6800ae-5406-4378-9033-3e4d075697d3'), 'developer/create repo-wide RADNI TAKT VUK supplemental visual reference mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.thematicSignals.join(',') === 'developer-and-create-vrh,radni-takt-repo-wide-reflection,vuk,bounded-vocabulary-extrimli-extrondol-extrem-dok-duk-dak-dik-for,audit-safe-summary-only', 'developer/create repo-wide RADNI TAKT VUK thematic signals mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create repo-wide RADNI TAKT VUK EXTREM ownership mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create repo-wide RADNI TAKT VUK EXTRONDOL ownership mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create repo-wide RADNI TAKT VUK SPAJA KOD boundary mismatch');
    assert(developerCreateRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create repo-wide RADNI TAKT VUK supplemental readiness status mismatch');
    ...
    const developerCreateKraljevskaVodicaSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'covecanstvo-kraljevska-vodica-zakon-silnog-developer-create');
    assert(Boolean(developerCreateKraljevskaVodicaSupplemental), 'developer/create KRALJEVSKA VODICA supplemental narrative id mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.imageToSignalProfile.scenarioId === 'covecanstvo-kraljevska-vodica-pravo-etika-mir-developer-create', 'developer/create KRALJEVSKA VODICA supplemental scenario id mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.visualReference.includes('e1d0a813-ed72-43ff-a943-112af972872d'), 'developer/create KRALJEVSKA VODICA supplemental visual reference mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.thematicSignals.join(',') === 'legal-governance-epilog,ethics-and-justice,nenarusavaj-mir,civic-order,bounded-non-enforcement', 'developer/create KRALJEVSKA VODICA thematic signals mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.citation.includes('nikada ne postaje runtime enforcement logika'), 'developer/create KRALJEVSKA VODICA citation must preserve non-enforcement boundary');
    assert(developerCreateKraljevskaVodicaSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create KRALJEVSKA VODICA EXTREM ownership mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create KRALJEVSKA VODICA EXTRONDOL ownership mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create KRALJEVSKA VODICA SPAJA KOD boundary mismatch');
    assert(developerCreateKraljevskaVodicaSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create KRALJEVSKA VODICA supplemental readiness status mismatch');
    const developerCreateKraljevskiRadTemplateSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevski-rad-digitalna-industrija-kompanija-spaja-developer-create');
    assert(Boolean(developerCreateKraljevskiRadTemplateSupplemental), 'developer/create KRALJEVSKI RAD TEMPLATE supplemental narrative id mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.imageToSignalProfile.scenarioId === 'kraljevski-rad-digitalna-industrija-znanje-tehnologija-ljudi-priroda-buducnost-developer-create', 'developer/create KRALJEVSKI RAD TEMPLATE supplemental scenario id mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.visualReference.includes('b71eec10-4557-4458-8686-60a739c91734'), 'developer/create KRALJEVSKI RAD TEMPLATE supplemental visual reference mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.thematicSignals.join(',') === 'kraljevski-rad,digitalna-industrija,kompanija-spaja,znanje,tehnologija,ljudi,priroda,buducnost', 'developer/create KRALJEVSKI RAD TEMPLATE thematic signals mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create KRALJEVSKI RAD TEMPLATE EXTREM ownership mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create KRALJEVSKI RAD TEMPLATE EXTRONDOL ownership mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create KRALJEVSKI RAD TEMPLATE SPAJA KOD boundary mismatch');
    assert(developerCreateKraljevskiRadTemplateSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create KRALJEVSKI RAD TEMPLATE supplemental readiness status mismatch');
    ...
    const developerCreateProfesionalnaGlobalnaKampanjaKraljevstvoSupplemental = report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevstvo-profesionalna-globalna-kampanja-nikola-spajic-developer-create');
    assert(Boolean(developerCreateProfesionalnaGlobalnaKampanjaKraljevstvoSupplemental), 'developer/create KRALJEVSTVO professional global campaign supplemental narrative id mismatch');
    assert(developerCreateProfesionalnaGlobalnaKampanjaKraljevstvoSupplemental?.imageToSignalProfile.scenarioId === 'kraljevstvo-profesionalna-globalna-kampanja-medijska-strategija-developer-create', 'developer/create KRALJEVSTVO professional global campaign supplemental scenario id mismatch');
    assert(developerCreateProfesionalnaGlobalnaKampanjaKraljevstvoSupplemental?.visualReference.includes('c9414c36-7876-43ee-ae39-fad8cd2622ed'), 'developer/create KRALJEVSTVO professional global campaign supplemental visual reference mismatch');
    ...
    assert(developerCreateGilskultureKraljevstvoSupplemental?.visualReference.includes('50cb9759-5ce2-490f-bcb2-8a3b73fed39f'), 'developer/create GILSKULTURE supplemental visual reference mismatch');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.thematicSignals.join(',') === 'kraljevstvo,covecanstvo-epilog,znanje-citanje,deca-buduci-narastaji,priroda-covek-tehnologija-u-ravnotezi,mir-pravda-odgovornost', 'developer/create GILSKULTURE thematic signals mismatch');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.citation.includes('“zakon silnog”'), 'developer/create GILSKULTURE citation must preserve source-text-only law-of-force boundary');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create GILSKULTURE EXTREM ownership mismatch');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create GILSKULTURE EXTRONDOL ownership mismatch');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create GILSKULTURE SPAJA KOD boundary mismatch');
    assert(developerCreateGilskultureKraljevstvoSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.readiness.status, 'developer/create GILSKULTURE supplemental readiness status mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.covecnostAuditVisualReference.packageOutputs.publicSummary.includes('URL-locked pending-title'), 'developer/create package public summary should mention URL-locked visuals');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.v700Extension.roadmapStageId === 'v700-apdejt-na-verziju-700', 'developer/create EXTREM V700 roadmap extension mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.v700Extension.noNewRuntimeRoutes, 'developer/create EXTREM V700 must keep no-new-runtime-routes lock');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.v700Extension.humanReviewStatus === 'required-before-promotion', 'developer/create EXTREM V700 human review lock mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.napoleonDiskaveriSelectionTrack.randomSelectionPosture.selectionChannel === 'napoleon-diskaveri-bounded-selection', 'developer/create EXTREM random-selection channel mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.napoleonDiskaveriSelectionTrack.randomSelectionPosture.reviewPosture === report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.napoleonDiskaveriSelectionTrack.humanReviewPosture, 'developer/create EXTREM random-selection review posture mismatch');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.currentImplementationStage.acceptanceEvidence.includes('dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori'), 'developer/create EXTREM acceptance evidence must include INSPEKTORI');
    ...
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.parentLegalTrack === 'KRALJEVSKI PRAVNI UNIVERZITET', 'developer/create INSPEKTORI parent legal track mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.innerControlModel.forbiddenOperationalArtifacts.includes('tactical-instruction'), 'developer/create INSPEKTORI forbidden artifact mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.universityCatalog.activeCount === 5, 'developer/create INSPEKTORI active university count mismatch');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.justicePath.humanReviewRequired, 'developer/create INSPEKTORI must always require human review');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.justicePath.evidentiaryCompletenessStatus === 'WATCH', 'developer/create INSPEKTORI evidentiary completeness should stay bounded when evidence inputs degrade without hard blockers');
    assert(report.dokDikDakDukConsistencyHealth.developerAndCreateRepoWideReflection.inspektori.justicePath.justicePathConsistency === 'BLOCKED', 'developer/create INSPEKTORI justice path should still block on deterministic fallback');
