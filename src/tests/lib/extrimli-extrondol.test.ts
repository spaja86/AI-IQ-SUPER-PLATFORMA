...
async function runTests(): Promise<void> {
  ...
  await test('report exposes Developer/Create lock and downstream sync expectations', () => {
    ...
    const vinogradiGrckaRestoranTrack =
      report.developerAndCreateRepoWideReflection.vinogradiGrckaRestoranTrack;
    ...
    assert(vinogradiGrckaRestoranTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == VINOGRADI GRČKA, RESTORAN', 'developer/create governance VINOGRADI alias mismatch');
    assert(vinogradiGrckaRestoranTrack.governancePosture.transitionType === 'executive-director-replacement-governance-scenario', 'developer/create governance VINOGRADI transition type mismatch');
    assert(vinogradiGrckaRestoranTrack.governancePosture.incomingDirectors.join(',') === 'Jonačić Slaviša,Jonačić Marko', 'developer/create governance VINOGRADI incoming directors mismatch');
    assert(vinogradiGrckaRestoranTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance VINOGRADI human review status mismatch');
    assert(vinogradiGrckaRestoranTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.vinogradiGrckaRestoranTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.vinogradiGrckaRestoranTrack,spajaKod.publicSignals.vinogradiGrckaRestoranStatus,spajaKod.developerAndCreateImplementationPackage.vinogradiGrckaRestoranSummary', 'developer/create governance VINOGRADI acceptance evidence mismatch');
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health stays deterministic across technical and governance layers', () => {
    ...
    assert(report.spajaKod.publicSignals.vinogradiGrckaRestoranStatus === report.developerAndCreateRepoWideReflection.vinogradiGrckaRestoranTrack.readinessSignal.status, 'SPAJA KOD VINOGRADI status mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.vinogradiGrckaRestoranSummary.governancePosture.incomingDirectors.join(',') === 'Jonačić Slaviša,Jonačić Marko', 'SPAJA KOD VINOGRADI executive transition incoming directors mismatch');
