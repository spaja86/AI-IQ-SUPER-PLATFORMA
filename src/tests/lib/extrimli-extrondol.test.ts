...
async function runTests(): Promise<void> {
  ...
  await test('report includes bounded orchestration score and degraded policy', () => {
    ...
    assert(releaseAuditKraljevstvoZvanicnoPravoLiceSupplemental?.visualReference.includes('82f065a8-3bd7-4c09-8d9f-9c66b8443d66'), 'developer/create release audit KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) supplemental visual reference mismatch');
    assert(releaseAuditKraljevstvoZvanicnoPravoLiceSupplemental?.thematicSignals.join(',') === 'kraljevstvo,zajednistvo,znanje,priroda,tehnologija,humanost', 'developer/create release audit KRALJEVSTVO (ZVANIČNO MOJE PRAVO LICE) thematic signals mismatch');
    ...
    const releaseAuditPendingSecondVisualSupplemental = report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(Boolean(releaseAuditPendingSecondVisualSupplemental), 'developer/create release audit pending second visual supplemental narrative id mismatch');
    assert(releaseAuditPendingSecondVisualSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-drugi-vizuel-pending-title-and-citation-confirmation', 'developer/create release audit pending second visual supplemental scenario id mismatch');
    assert(releaseAuditPendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'developer/create release audit pending second visual supplemental visual reference mismatch');
    assert(releaseAuditPendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'developer/create release audit pending second visual thematic signals mismatch');
    assert(releaseAuditPendingSecondVisualSupplemental?.citation.includes('dok naziv i citat ne budu ljudski potvrđeni'), 'developer/create release audit pending second visual citation mismatch');
    assert(releaseAuditPendingSecondVisualSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.status, 'developer/create release audit pending second visual supplemental readiness status mismatch');
    ...
    assert(
      report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.acceptanceEvidence.join(',') ===
        'releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.implementationPackage,developerAndCreateRepoWideReflection,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIdentityFinanceGovernance,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.companionAuditVisualReferences,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.osnoveRispektProtocol,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiPlateGovernance,spajaKod.publicSignals.developerAndCreateStatus,spajaKod.publicSignals.developerAndCreateImplementationStatus,spajaKod.publicSignals.aiPlateStatus',
    ...
    const spajaKodPendingSecondVisualSupplemental = report.spajaKod.developerAndCreateVisualReflection.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'developer-create-drugi-vizuel-pending-human-confirmation');
    assert(spajaKodPendingSecondVisualSupplemental?.visualReference.includes('76a37bff-98e0-4583-9580-b34310b27aa8'), 'SPAJA KOD developer/create pending second visual supplemental visual reference mismatch');
    assert(spajaKodPendingSecondVisualSupplemental?.thematicSignals.join(',') === 'pending-confirmation,documentation-only-slot,audit-safe-boundary,human-review-required,summary-only-downstream', 'SPAJA KOD developer/create pending second visual thematic signals mismatch');
