...
async function runTests(): Promise<void> {
  ...
  await test('report exposes Developer/Create lock and downstream sync expectations', () => {
    ...
    assert(['READY', 'WATCH', 'BLOCKED'].includes(report.spajaKod.publicSignals.innovationRegistryStatus), 'SPAJA KOD innovation status mismatch');
    ...
    const kraljevskiPokloniZaSvacijiRodjendanTrack =
      report.developerAndCreateRepoWideReflection.kraljevskiPokloniZaSvacijiRodjendanTrack;
    const kraljevskiRadTrack =
      report.developerAndCreateRepoWideReflection.kraljevskiRadTrack;
    const radniProstorTrack = report.developerAndCreateRepoWideReflection.radniProstorTrack;
    const konstrukcijeIProjektovanjeTrack = report.developerAndCreateRepoWideReflection.konstrukcijeIProjektovanjeTrack;
    const vinogradiGrockaRestoranTrack = report.developerAndCreateRepoWideReflection.vinogradiGrockaRestoranTrack;
    const poslovnaPonudaZelezaraDooTrack = report.developerAndCreateRepoWideReflection.poslovnaPonudaZelezaraDooTrack;
    const aiIqKonferencijaZaStampuTrack = report.developerAndCreateRepoWideReflection.aiIqKonferencijaZaStampuTrack;
    const radioTrack = report.developerAndCreateRepoWideReflection.radioTrack;
    const muzickaKutijaTrack = report.developerAndCreateRepoWideReflection.muzickaKutijaTrack;
    const aiIqLaboratorijaTrack = report.developerAndCreateRepoWideReflection.aiIqLaboratorijaTrack;
    ...
    assert(radioTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == RADIO', 'developer/create governance RADIO alias mismatch');
    assert(radioTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance RADIO source mismatch');
    assert(radioTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance RADIO governance source mismatch');
    assert(radioTrack.mikrofonProjectionAlias.canonicalEquality === 'MIKROFON == MEGAFON, DISTRIBUTER, SAKSOFON', 'developer/create governance RADIO mikrofon projection equality mismatch');
    assert(radioTrack.mikrofonProjectionAlias.reviewPosture === radioTrack.reviewPosture || ['ALIGNED', 'WATCH', 'REVIEW_REQUIRED'].includes(radioTrack.mikrofonProjectionAlias.reviewPosture), 'developer/create governance RADIO mikrofon projection review posture mismatch');
    assert(radioTrack.mikrofonProjectionAlias.summarySafePublicFields.join(',') === 'canonicalEquality,status,blockerReason,watchReasons,reviewPosture,downstreamReference,mikrofonSummary,semanticPreservation', 'developer/create governance RADIO mikrofon projection summary-safe fields mismatch');
    assert(radioTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.radioTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radioTrack,spajaKod.publicSignals.radioStatus,spajaKod.developerAndCreateImplementationPackage.radioSummary', 'developer/create governance RADIO acceptance evidence mismatch');
    assert(muzickaKutijaTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == MUZIČKA KUTIJA', 'developer/create governance MUZIČKA KUTIJA alias mismatch');
    assert(muzickaKutijaTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance MUZIČKA KUTIJA source mismatch');
    assert(muzickaKutijaTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance MUZIČKA KUTIJA governance source mismatch');
    assert(muzickaKutijaTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.muzickaKutijaTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.muzickaKutijaTrack,spajaKod.publicSignals.muzickaKutijaStatus,spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary', 'developer/create governance MUZIČKA KUTIJA acceptance evidence mismatch');
    assert(muzickaKutijaTrack.rolloutPlan.includes('additive-only bounded audio/instrument alias'), 'developer/create governance MUZIČKA KUTIJA rollout plan mismatch');
    assert(muzickaKutijaTrack.rollbackPlan.includes('bounded instrument mapping'), 'developer/create governance MUZIČKA KUTIJA rollback plan mismatch');
    assert(
    ...
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == PROMOCIJE / TIKETI / BONUSI / PROPUSNICE / ADMINISTRATIVNI BONUSI', 'developer/create governance promotions package alias mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance promotions package source mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance promotions package governance source mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack,spajaKod.publicSignals.promocijeTiketiBonusiPropusniceAdministrativniBonusiStatus,spajaKod.developerAndCreateImplementationPackage.promocijeTiketiBonusiPropusniceAdministrativniBonusiSummary', 'developer/create governance promotions package acceptance evidence mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.governancePosture.administrativeBonusOverride === 'human-review-gated-override', 'developer/create governance promotions package admin override posture mismatch');
    assert(promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance promotions package human review status mismatch');
    const kraljevskoTakmicenjeTrack =
      report.developerAndCreateRepoWideReflection.kraljevskoTakmicenjeTrack;
    assert(kraljevskoTakmicenjeTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KRALJEVSKO TAKMIČENJE', 'developer/create governance kraljevsko takmicenje alias mismatch');
    assert(kraljevskoTakmicenjeTrack.governancePosture.tournamentModel === 'round-robin-svako-sa-svakim', 'developer/create governance kraljevsko takmicenje tournament model mismatch');
    assert(kraljevskoTakmicenjeTrack.governancePosture.rewardsPolicy === 'top-100-audit-governed', 'developer/create governance kraljevsko takmicenje rewards policy mismatch');
    assert(kraljevskoTakmicenjeTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance kraljevsko takmicenje human review status mismatch');
    assert(kraljevskiPokloniZaSvacijiRodjendanTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KRALJEVSKI POKLONI ZA SVAČIJI ROĐENDAN', 'developer/create governance kraljevski pokloni alias mismatch');
    assert(kraljevskiPokloniZaSvacijiRodjendanTrack.governancePosture.ownershipSplit === 'DOK-DIK-FOR=EXTREM|DAK-DUK=EXTRONDOL|SPAJA-KOD=SUMMARY', 'developer/create governance kraljevski pokloni ownership split mismatch');
    assert(kraljevskiPokloniZaSvacijiRodjendanTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance kraljevski pokloni human review status mismatch');
    assert(kraljevskiPokloniZaSvacijiRodjendanTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.kraljevskiPokloniZaSvacijiRodjendanTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiPokloniZaSvacijiRodjendanTrack,spajaKod.publicSignals.kraljevskiPokloniZaSvacijiRodjendanStatus,spajaKod.developerAndCreateImplementationPackage.kraljevskiPokloniZaSvacijiRodjendanSummary', 'developer/create governance kraljevski pokloni acceptance evidence mismatch');
    assert(kraljevskiRadTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KRALJEVSKI RAD', 'developer/create governance kraljevski rad alias mismatch');
    assert(kraljevskiRadTrack.governancePosture.sequenceOrderRule === 'strict-order-required', 'developer/create governance kraljevski rad sequence-order rule mismatch');
    assert(kraljevskiRadTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance kraljevski rad human review status mismatch');
    assert(kraljevskiRadTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.kraljevskiRadTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiRadTrack,spajaKod.publicSignals.kraljevskiRadStatus,spajaKod.developerAndCreateImplementationPackage.kraljevskiRadSummary', 'developer/create governance kraljevski rad acceptance evidence mismatch');
    assert(radniProstorTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == RADNI PROSTOR', 'developer/create governance RADNI PROSTOR alias mismatch');
    assert(radniProstorTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance RADNI PROSTOR source mismatch');
    assert(radniProstorTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance RADNI PROSTOR governance source mismatch');
    assert(radniProstorTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.radniProstorTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radniProstorTrack,spajaKod.publicSignals.radniProstorStatus,spajaKod.developerAndCreateImplementationPackage.radniProstorSummary', 'developer/create governance RADNI PROSTOR acceptance evidence mismatch');
    assert(aiIqLaboratorijaTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == AI IQ LABORATORIJA == LABORATORIJSKI NALAZI FAUNE I FLORE I GRAĐEVINSKOG MATERIJALA', 'developer/create governance AI IQ LABORATORIJA alias mismatch');
    assert(aiIqLaboratorijaTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance AI IQ LABORATORIJA source mismatch');
    assert(aiIqLaboratorijaTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance AI IQ LABORATORIJA governance source mismatch');
    assert(aiIqLaboratorijaTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.aiIqLaboratorijaTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqLaboratorijaTrack,spajaKod.publicSignals.aiIqLaboratorijaStatus,spajaKod.developerAndCreateImplementationPackage.aiIqLaboratorijaSummary', 'developer/create governance AI IQ LABORATORIJA acceptance evidence mismatch');
    assert(konstrukcijeIProjektovanjeTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == KONSTRUKCIJE I PROJEKTOVANJE', 'developer/create governance KONSTRUKCIJE I PROJEKTOVANJE alias mismatch');
    assert(konstrukcijeIProjektovanjeTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance KONSTRUKCIJE I PROJEKTOVANJE source mismatch');
    assert(konstrukcijeIProjektovanjeTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance KONSTRUKCIJE I PROJEKTOVANJE governance source mismatch');
    assert(konstrukcijeIProjektovanjeTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.konstrukcijeIProjektovanjeTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.konstrukcijeIProjektovanjeTrack,spajaKod.publicSignals.konstrukcijeIProjektovanjeStatus,spajaKod.developerAndCreateImplementationPackage.konstrukcijeIProjektovanjeSummary', 'developer/create governance KONSTRUKCIJE I PROJEKTOVANJE acceptance evidence mismatch');
    assert(vinogradiGrockaRestoranTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == VINOGRADI GROCKA, RESTORAN', 'developer/create governance VINOGRADI GROCKA, RESTORAN alias mismatch');
    assert(vinogradiGrockaRestoranTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance VINOGRADI GROCKA, RESTORAN source mismatch');
    assert(vinogradiGrockaRestoranTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance VINOGRADI GROCKA, RESTORAN governance source mismatch');
    assert(vinogradiGrockaRestoranTrack.humanReviewStatus === 'required-before-promotion', 'developer/create governance VINOGRADI GROCKA, RESTORAN human review status mismatch');
    assert(vinogradiGrockaRestoranTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.vinogradiGrockaRestoranTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.vinogradiGrockaRestoranTrack', 'developer/create governance VINOGRADI GROCKA, RESTORAN acceptance evidence mismatch');
    assert(vinogradiGrockaRestoranTrack.rolloutPlan.includes('additive-only leadership-transition governance package'), 'developer/create governance VINOGRADI GROCKA, RESTORAN rollout plan mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO alias mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.sourceOfTruth === '/api/extrimli/extrem', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO source mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.governanceSource === '/api/extrimli/extrondol', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO governance source mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.legalComplianceReviewStatus === 'required-before-promotion', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO legal/compliance review status mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.kpiImpactStatus === 'required-before-promotion', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO KPI review status mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.releaseAuditSummaryRequired, 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO release-audit requirement mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.poslovnaPonudaZelezaraDooTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.poslovnaPonudaZelezaraDooTrack,spajaKod.publicSignals.poslovnaPonudaZelezaraDooStatus,spajaKod.developerAndCreateImplementationPackage.poslovnaPonudaZelezaraDooSummary', 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO acceptance evidence mismatch');
    assert(poslovnaPonudaZelezaraDooTrack.urgentMeetingIntakePackage.privacyCompliance.phoneNumbersPublicSummaryAllowed === false, 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO must keep phone numbers out of public summary');
    assert(poslovnaPonudaZelezaraDooTrack.urgentMeetingIntakePackage.privacyCompliance.phoneNumbersDownstreamSyncAllowed === false, 'developer/create governance POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO must keep phone numbers out of downstream sync');
    assert(report.spajaKod.publicSignals.poslovnaPonudaZelezaraDooStatus === poslovnaPonudaZelezaraDooTrack.readinessSignal.status, 'SPAJA KOD POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO summary mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.poslovnaPonudaZelezaraDooSummary.personalContactsRedacted, 'SPAJA KOD POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO summary must remain redacted');
    assert(audioVisualKontrabasPackage.acceptanceEvidence.join(',') === 'developerAndCreateRepoWideReflection.audioVisualKontrabasPackage,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.audioVisualKontrabasPackage,spajaKod.publicSignals.developerAndCreateAudioVisualStatus,spajaKod.developerAndCreateVisualReflection.audioVisualKontrabasPackage', 'developer/create governance audio-visual package acceptance evidence mismatch');
  ...
  await test('report includes bounded orchestration score and degraded policy', () => {
    ...
    const releaseAuditKraljevskiRadTemplateSupplemental = report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevski-rad-digitalna-industrija-kompanija-spaja-developer-create');
    assert(Boolean(releaseAuditKraljevskiRadTemplateSupplemental), 'developer/create release audit KRALJEVSKI RAD TEMPLATE supplemental narrative id mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.imageToSignalProfile.scenarioId === 'kraljevski-rad-digitalna-industrija-znanje-tehnologija-ljudi-priroda-buducnost-developer-create', 'developer/create release audit KRALJEVSKI RAD TEMPLATE supplemental scenario id mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.visualReference.includes('b71eec10-4557-4458-8686-60a739c91734'), 'developer/create release audit KRALJEVSKI RAD TEMPLATE supplemental visual reference mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.thematicSignals.join(',') === 'kraljevski-rad,digitalna-industrija,kompanija-spaja,znanje,tehnologija,ljudi,priroda,buducnost', 'developer/create release audit KRALJEVSKI RAD TEMPLATE thematic signals mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create release audit KRALJEVSKI RAD TEMPLATE EXTREM ownership mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create release audit KRALJEVSKI RAD TEMPLATE EXTRONDOL ownership mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create release audit KRALJEVSKI RAD TEMPLATE SPAJA KOD boundary mismatch');
    assert(releaseAuditKraljevskiRadTemplateSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.status, 'developer/create release audit KRALJEVSKI RAD TEMPLATE supplemental readiness status mismatch');
    ...
    const releaseAuditRepoWideRadniTaktVukSupplemental = report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'developer-create-vrh-radni-takt-svemu-u-repozitorijumu-vuk-developer-create');
    assert(Boolean(releaseAuditRepoWideRadniTaktVukSupplemental), 'developer/create release audit repo-wide RADNI TAKT VUK supplemental narrative id mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.scenarioId === 'developer-create-vrh-radni-takt-da-se-odrazi-na-svemu-u-repozitorijumu-vuk-developer-create', 'developer/create release audit repo-wide RADNI TAKT VUK supplemental scenario id mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.visualReference.includes('de6800ae-5406-4378-9033-3e4d075697d3'), 'developer/create release audit repo-wide RADNI TAKT VUK supplemental visual reference mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.thematicSignals.join(',') === 'developer-and-create-vrh,radni-takt-repo-wide-reflection,vuk,bounded-vocabulary-extrimli-extrondol-extrem-dok-duk-dak-dik-for,audit-safe-summary-only', 'developer/create release audit repo-wide RADNI TAKT VUK thematic signals mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.dokDikFor === 'EXTREM', 'developer/create release audit repo-wide RADNI TAKT VUK EXTREM ownership mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.dakDuk === 'EXTRONDOL', 'developer/create release audit repo-wide RADNI TAKT VUK EXTRONDOL ownership mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'developer/create release audit repo-wide RADNI TAKT VUK SPAJA KOD boundary mismatch');
    assert(releaseAuditRepoWideRadniTaktVukSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.status, 'developer/create release audit repo-wide RADNI TAKT VUK supplemental readiness status mismatch');
    ...
    const releaseAuditPolitickaKompanjaKraljevstvoSupplemental = report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevstvo-covecanstvo-kralj-nad-kraljevima-spajic-nikola-politicka-kompanja-ekstremno-maksimalno-profesionalna-developer-create');
    assert(Boolean(releaseAuditPolitickaKompanjaKraljevstvoSupplemental), 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental narrative id mismatch');
    assert(releaseAuditPolitickaKompanjaKraljevstvoSupplemental?.imageToSignalProfile.scenarioId === 'kraljevstvo-covecanstvo-politicka-kompanja-ekstremno-maksimalno-profesionalna-audit-safe-developer-create', 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental scenario id mismatch');
    assert(releaseAuditPolitickaKompanjaKraljevstvoSupplemental?.visualReference === 'documentation-only://kraljevstvo-covecanstvo-kralj-nad-kraljevima-spajic-nikola-politicka-kompanja-ekstremno-maksimalno-profesionalna', 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental visual reference mismatch');
    assert(releaseAuditPolitickaKompanjaKraljevstvoSupplemental?.thematicSignals.join(',') === 'kraljevstvo-covecanstvo,kralj-nad-kraljevima,spajic-nikola-public-presentation,politicka-kompanja-ekstremno-maksimalno-profesionalna,dok-dik-for-extrem,dak-duk-extrondol,spaja-kod-summary-only,audit-safe-release-governance', 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign thematic signals mismatch');
    assert(releaseAuditPolitickaKompanjaKraljevstvoSupplemental?.citation.includes('human-review'), 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign citation mismatch');
    assert(releaseAuditPolitickaKompanjaKraljevstvoSupplemental?.imageToSignalProfile.signalOutputs.readinessStatus === report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.status, 'developer/create release audit KRALJEVSTVO/ČOVEČANSTVO political campaign supplemental readiness status mismatch');
    ...
    assert(report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.v700Extension.roadmapStageId === 'v700-apdejt-na-verziju-700', 'developer/create release audit V700 roadmap extension mismatch');
    assert(report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.v700Extension.humanReviewStatus === 'required-before-promotion', 'developer/create release audit V700 human review lock mismatch');
    ...
    const releaseAuditAcceptanceEvidence =
      report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.acceptanceEvidence;
    assert(releaseAuditAcceptanceEvidence.includes('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqLaboratorijaTrack'), 'developer/create release audit acceptance evidence must include AI IQ LABORATORIJA governance track');
    assert(releaseAuditAcceptanceEvidence.includes('spajaKod.publicSignals.aiIqLaboratorijaStatus'), 'developer/create release audit acceptance evidence must include SPAJA KOD AI IQ LABORATORIJA status');
    assert(
      releaseAuditAcceptanceEvidence.indexOf('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqLaboratorijaTrack')
        > releaseAuditAcceptanceEvidence.indexOf('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiRadTrack')
      && releaseAuditAcceptanceEvidence.indexOf('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radioTrack')
        > releaseAuditAcceptanceEvidence.indexOf('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqKonferencijaZaStampuTrack'),
      'developer/create release audit acceptance evidence must place AI IQ LABORATORIJA after KRALJEVSKI RAD and RADIO after AI IQ press tracks',
    );
    assert(releaseAuditAcceptanceEvidence.includes('releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.muzickaKutijaTrack'), 'developer/create release audit acceptance evidence must include MUZIČKA KUTIJA governance track');
    assert(releaseAuditAcceptanceEvidence.includes('spajaKod.publicSignals.muzickaKutijaStatus'), 'developer/create release audit acceptance evidence must include SPAJA KOD MUZIČKA KUTIJA status');
    assert(
      report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.roadmapExecution.acceptanceEvidence.join(',') ===
        'releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.implementationPackage,developerAndCreateRepoWideReflection,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.audioVisualKontrabasPackage,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiProgramskiUneverzitet,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiDrustveniPoredak,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiDrustveniPoredak.kraljevskiAktBezbednosti,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiBastaUneverzite,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIdentityFinanceGovernance,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.supplementalVisualReferences,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.covecnostAuditVisualReference.companionAuditVisualReferences,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.osnoveRispektProtocol,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiPlateGovernance,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.napoleonDiskaveriSelectionTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.eksperimentProgramskiJezikTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.sarkazamPrivrednaGranaDigitalizmaTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.notes1450Track,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.saradnjaReadyPackage,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.leksikonTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radniProstorTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.kraljevskiRadTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqLaboratorijaTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.konstrukcijeIProjektovanjeTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.poslovnaPonudaZelezaraDooTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.aiIqKonferencijaZaStampuTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.radioTrack,releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.muzickaKutijaTrack,spajaKod.publicSignals.developerAndCreateStatus,spajaKod.publicSignals.developerAndCreateAudioVisualStatus,spajaKod.publicSignals.developerAndCreateImplementationStatus,spajaKod.publicSignals.napoleonDiskaveriStatus,spajaKod.publicSignals.eksperimentProgramskiJezikStatus,spajaKod.publicSignals.sarkazamPrivrednaGranaDigitalizmaStatus,spajaKod.publicSignals.notes1450Status,spajaKod.publicSignals.saradnjaReadyStatus,spajaKod.publicSignals.leksikonStatus,spajaKod.publicSignals.promocijeTiketiBonusiPropusniceAdministrativniBonusiStatus,spajaKod.publicSignals.radniProstorStatus,spajaKod.publicSignals.kraljevskiRadStatus,spajaKod.publicSignals.aiIqLaboratorijaStatus,spajaKod.publicSignals.konstrukcijeIProjektovanjeStatus,spajaKod.publicSignals.poslovnaPonudaZelezaraDooStatus,spajaKod.publicSignals.aiIqKonferencijaZaStampuStatus,spajaKod.publicSignals.radioStatus,spajaKod.publicSignals.muzickaKutijaStatus,spajaKod.publicSignals.aiPlateStatus',
    ...
    const spajaKodKraljevskiRadTemplateSupplemental = report.spajaKod.developerAndCreateVisualReflection.supplementalVisualReferences.find((reference) => reference.canonicalNarrativeId === 'kraljevski-rad-digitalna-industrija-kompanija-spaja-developer-create');
    assert(spajaKodKraljevskiRadTemplateSupplemental?.visualReference.includes('b71eec10-4557-4458-8686-60a739c91734'), 'SPAJA KOD developer/create KRALJEVSKI RAD TEMPLATE supplemental visual reference mismatch');
    assert(spajaKodKraljevskiRadTemplateSupplemental?.imageToSignalProfile.scenarioId === 'kraljevski-rad-digitalna-industrija-znanje-tehnologija-ljudi-priroda-buducnost-developer-create', 'SPAJA KOD developer/create KRALJEVSKI RAD TEMPLATE supplemental scenario id mismatch');
    assert(spajaKodKraljevskiRadTemplateSupplemental?.thematicSignals.join(',') === 'kraljevski-rad,digitalna-industrija,kompanija-spaja,znanje,tehnologija,ljudi,priroda,buducnost', 'SPAJA KOD developer/create KRALJEVSKI RAD TEMPLATE thematic signals mismatch');
    assert(spajaKodKraljevskiRadTemplateSupplemental?.imageToSignalProfile.ownershipLock.spajaKod === 'audit-safe-summary-only', 'SPAJA KOD developer/create KRALJEVSKI RAD TEMPLATE boundary mismatch');
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health stays deterministic across technical and governance layers', () => {
    ...
    assert(report.developerAndCreateRepoWideReflection.roadmapExecution.v700Extension.roadmapStageId === 'v700-apdejt-na-verziju-700', 'developer/create governance V700 roadmap extension mismatch');
    assert(report.developerAndCreateRepoWideReflection.roadmapExecution.v700Extension.noParallelSourceOfTruth, 'developer/create governance V700 must keep no-parallel-source-of-truth lock');
    ...
    assert(report.spajaKod.developerAndCreateImplementationPackage.napoleonDiskaveriSummary.randomSelectionPosture.requestAlias === 'RANDOM selekcija svega', 'SPAJA KOD developer/create random-selection alias mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.napoleonDiskaveriSummary.randomSelectionPosture.selectionChannel === 'napoleon-diskaveri-bounded-selection', 'SPAJA KOD developer/create random-selection channel mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.napoleonDiskaveriSummary.randomSelectionPosture.reviewPosture === report.spajaKod.developerAndCreateImplementationPackage.napoleonDiskaveriSummary.humanReviewPosture, 'SPAJA KOD developer/create random-selection review posture mismatch');
    ...
    assert(report.spajaKod.publicSignals.leksikonStatus === report.developerAndCreateRepoWideReflection.leksikonTrack.readinessSignal.status, 'SPAJA KOD LEKSIKON status mismatch');
    assert(report.spajaKod.publicSignals.promocijeTiketiBonusiPropusniceAdministrativniBonusiStatus === report.developerAndCreateRepoWideReflection.promocijeTiketiBonusiPropusniceAdministrativniBonusiTrack.readinessSignal.status, 'SPAJA KOD promotions package status mismatch');
    assert(report.spajaKod.publicSignals.radniProstorStatus === report.developerAndCreateRepoWideReflection.radniProstorTrack.readinessSignal.status, 'SPAJA KOD RADNI PROSTOR status mismatch');
    assert(report.spajaKod.publicSignals.kraljevskiRadStatus === report.developerAndCreateRepoWideReflection.kraljevskiRadTrack.readinessSignal.status, 'SPAJA KOD KRALJEVSKI RAD status mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.kraljevskiRadSummary.tokenOrderStatus === report.developerAndCreateRepoWideReflection.kraljevskiRadTrack.readinessSignal.tokenOrderStatus, 'SPAJA KOD KRALJEVSKI RAD token-order summary mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.kraljevskiRadSummary.duplicateRuleStatus === report.developerAndCreateRepoWideReflection.kraljevskiRadTrack.readinessSignal.duplicateRuleStatus, 'SPAJA KOD KRALJEVSKI RAD duplicate-rule summary mismatch');
    assert(report.spajaKod.publicSignals.konstrukcijeIProjektovanjeStatus === report.developerAndCreateRepoWideReflection.konstrukcijeIProjektovanjeTrack.readinessSignal.status, 'SPAJA KOD KONSTRUKCIJE I PROJEKTOVANJE status mismatch');
    assert(report.spajaKod.publicSignals.poslovnaPonudaZelezaraDooStatus === report.developerAndCreateRepoWideReflection.poslovnaPonudaZelezaraDooTrack.readinessSignal.status, 'SPAJA KOD POSLOVNA PONUDA / ŽELEZARA D.O.O. SMEDEREVO status mismatch');
    assert(report.spajaKod.publicSignals.radioStatus === report.developerAndCreateRepoWideReflection.radioTrack.readinessSignal.status, 'SPAJA KOD RADIO status mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.radioSummary.mikrofonProjectionAlias.canonicalEquality === 'MIKROFON == MEGAFON, DISTRIBUTER, SAKSOFON', 'SPAJA KOD RADIO mikrofon projection equality mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.radioSummary.mikrofonProjectionAlias.publicBoundary === 'audit-safe-summary-only', 'SPAJA KOD RADIO mikrofon projection public boundary mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.radioSummary.mikrofonProjectionAlias.status === report.developerAndCreateRepoWideReflection.radioTrack.mikrofonProjectionAlias.readinessSignal.status, 'SPAJA KOD RADIO mikrofon projection status mismatch');
    assert(report.spajaKod.publicSignals.muzickaKutijaStatus === report.developerAndCreateRepoWideReflection.muzickaKutijaTrack.readinessSignal.status, 'SPAJA KOD MUZIČKA KUTIJA status mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary.canonicalAlias === 'DEVELOPER AND CREATE == VRH PROGRAMSKOG EKVILADENTA == MUZIČKA KUTIJA', 'SPAJA KOD MUZIČKA KUTIJA canonical alias mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary.publicBoundary === 'audit-safe-summary-only', 'SPAJA KOD MUZIČKA KUTIJA public boundary mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary.musicBoxSummary === report.developerAndCreateRepoWideReflection.muzickaKutijaTrack.musicBoxSummary, 'SPAJA KOD MUZIČKA KUTIJA summary mismatch');
    assert(report.spajaKod.developerAndCreateImplementationPackage.muzickaKutijaSummary.mappedAudioInstrumentLayers.instrumentTabla === 'PARADIJOGONALNO PROGRAMIRANJE', 'SPAJA KOD MUZIČKA KUTIJA instrument mapping mismatch');
