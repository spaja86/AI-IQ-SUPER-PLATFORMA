...
async function runTests(): Promise<void> {
  ...
  await test('DOK/DIK/DAK/DUK/FOR consistency health stays deterministic across technical and governance layers', () => {
    ...
    assert(report.releaseAuditSummary.developerAndCreateRepoWideReflectionGovernance.inspektori.escalatedReviewRequiredBeforeWideRollout, 'developer/create release-audit INSPEKTORI escalated rollout-review flag should stay true when justice-path evidence is blocked');
    assert(report.developerAndCreateRepoWideReflection.inspektori.justicePath.evidentiaryCompletenessStatus === 'BLOCKED', 'developer/create INSPEKTORI justice-path evidentiary completeness should stay blocked in the default payload');
    assert(report.developerAndCreateRepoWideReflection.inspektori.justicePath.reviewPosture === 'REVIEW_REQUIRED', 'developer/create INSPEKTORI justice-path posture should escalate when technical justice-path evidence is blocked');
