...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1043 Test (#1190)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1043', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1043, 'ciljBroj must be 1043');
  ...
  await test('TOTAL_API_ROUTES je >= 1043', () => {
    assert(TOTAL_API_ROUTES >= 1043, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1043`);
  });

  await test('Poruka sadrži 1043', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1043'), 'poruka mora sadržati 1043');
  });
