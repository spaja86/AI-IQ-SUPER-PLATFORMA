// Autofinish — spaja-unit-testovi Route Coverage Test
...
import { GET } from '../../app/api/spaja-unit-testovi/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-unit-testovi — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-unit-testovi/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-unit-testovi', {
