// Autofinish — digitalna-observatorija-status Route Coverage Test
...
import { GET } from '../../app/api/digitalna-observatorija-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 digitalna-observatorija-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/digitalna-observatorija-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    ...
    assert(typeof (body as Record<string, unknown>).status === 'string', 'status mora biti string');
  });
