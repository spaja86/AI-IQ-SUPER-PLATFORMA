// Autofinish #1296 — API Milestone 1044 Route Coverage Test
// Pokretanje: npx tsx src/tests/autofinish/autofinish-api-milestone-1044-route.test.ts

import fs from 'node:fs';
import path from 'node:path';
import { GET } from '../../app/api/autofinish-api-milestone-1044/route';
import {
  APP_VERSION,
  AUTOFINISH_COUNT,
  TOTAL_API_ROUTES,
...
async function runTests(): Promise<void> {
  console.log('\n🏁 Autofinish API Milestone 1044 — Route Coverage Test Suite (#1296)\n');

  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/autofinish-api-milestone-1044/route.ts');
  ...
  await test('Payload ima osnovna polja', () => {
    assertEqual(body['naziv'] as string, 'Autofinish API Milestone 1044', 'naziv');
  ...
  await test('milestone objekat ima ispravne vrednosti', () => {
    ...
    assertEqual(milestone['ciljBroj'] as number, 1044, 'ciljBroj');
