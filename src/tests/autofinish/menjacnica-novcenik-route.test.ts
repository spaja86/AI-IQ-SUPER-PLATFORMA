// Autofinish — menjacnica-novcenik Route Coverage Test
...
import { GET } from '../../app/api/menjacnica-novcenik/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica-novcenik — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica-novcenik/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica-novcenik', {
