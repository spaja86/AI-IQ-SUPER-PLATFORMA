// Autofinish — sitemap-status Route Coverage Test
...
import { GET } from '../../app/api/sitemap-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 sitemap-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/sitemap-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/sitemap-status', {
