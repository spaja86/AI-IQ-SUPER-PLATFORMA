// Autofinish — ekosistem-status Route Coverage Test
...
import { GET } from '../../app/api/ekosistem-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 ekosistem-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/ekosistem-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/ekosistem-status', {
