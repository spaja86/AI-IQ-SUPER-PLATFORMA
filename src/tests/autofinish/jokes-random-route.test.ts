// Autofinish — jokes/random Route Coverage Test
...
import { GET, POST } from '../../app/api/jokes/random/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 jokes/random — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/jokes/random/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/jokes/random', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/jokes/random', {
      method: 'POST',
