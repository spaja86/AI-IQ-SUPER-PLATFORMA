// Autofinish — prompt-status Route Coverage Test
...
import { GET } from '../../app/api/prompt-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 prompt-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/prompt-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/prompt-status', {
