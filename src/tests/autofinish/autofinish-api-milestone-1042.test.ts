...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1042 Test (#1189)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1042', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1042, 'ciljBroj must be 1042');
  ...
  await test('TOTAL_API_ROUTES je >= 1042', () => {
    assert(TOTAL_API_ROUTES >= 1042, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1042`);
  });

  await test('Poruka sadrži 1042', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1042'), 'poruka mora sadržati 1042');
  });
