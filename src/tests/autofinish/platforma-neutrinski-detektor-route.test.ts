// Autofinish — platforma-neutrinski-detektor Route Coverage Test
...
import { GET } from '../../app/api/platforma-neutrinski-detektor/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforma-neutrinski-detektor — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforma-neutrinski-detektor/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforma-neutrinski-detektor', {
