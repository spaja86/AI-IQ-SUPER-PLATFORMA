// Autofinish — io-openui-ao-laboratorija-status Route Coverage Test
...
import { GET } from '../../app/api/io-openui-ao-laboratorija-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 io-openui-ao-laboratorija-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/io-openui-ao-laboratorija-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/io-openui-ao-laboratorija-status', {
