// Autofinish — prompt-pregled Route Coverage Test
...
import { GET } from '../../app/api/prompt-pregled/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 prompt-pregled — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/prompt-pregled/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/prompt-pregled', {
