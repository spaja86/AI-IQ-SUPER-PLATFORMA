// Autofinish — dijagnostika-branch-cleanup Route Coverage Test
...
import { GET } from '../../app/api/dijagnostika-branch-cleanup/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 dijagnostika-branch-cleanup — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/dijagnostika-branch-cleanup/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/dijagnostika-branch-cleanup', {
