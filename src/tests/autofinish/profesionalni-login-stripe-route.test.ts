// Autofinish — profesionalni-login-stripe Route Coverage Test
...
import { GET, POST } from '../../app/api/profesionalni-login-stripe/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 profesionalni-login-stripe — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/profesionalni-login-stripe/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/profesionalni-login-stripe', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/profesionalni-login-stripe', {
      method: 'POST',
