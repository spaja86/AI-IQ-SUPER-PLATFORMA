...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1015 Test (#1171)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1015', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1015, 'ciljBroj must be 1015');
  ...
  await test('TOTAL_API_ROUTES je >= 1015', () => {
    assert(TOTAL_API_ROUTES >= 1015, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1015`);
  });

  await test('Poruka sadrži 1015', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1015'), 'poruka mora sadržati 1015');
  });
