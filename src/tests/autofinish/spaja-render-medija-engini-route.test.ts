// Autofinish — spaja-render-medija-engini Route Coverage Test
...
import { GET } from '../../app/api/spaja-render-medija-engini/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-render-medija-engini — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-render-medija-engini/route.ts');
  ...
  await test('GET smoke provera', async () => {
    ...
    if (isObject(body)) {
      ...
      assert(isObject(body['enginiPoKategoriji']), 'enginiPoKategoriji ostaje objekat');
      ...
      if (isObject(formula)) {
        ...
        if (isObject(slika) && isObject(video)) {
          ...
          assert(typeof slika['preporuka'] === 'string' && (slika['preporuka'] as string).length > 0, 'slika preporuka');
          assert(typeof video['preporuka'] === 'string' && (video['preporuka'] as string).length > 0, 'video preporuka');
        }
      }
    }
  });
