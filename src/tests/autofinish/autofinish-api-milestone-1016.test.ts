...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1016 Test (#1172)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1016', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1016, 'ciljBroj must be 1016');
  ...
  await test('TOTAL_API_ROUTES je >= 1016', () => {
    assert(TOTAL_API_ROUTES >= 1016, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1016`);
  });

  await test('Poruka sadrži 1016', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1016'), 'poruka mora sadržati 1016');
  });
