// Autofinish — spaja-pro-zasebni-endzin-analiza Route Coverage Test
...
import { GET, POST } from '../../app/api/spaja-pro-zasebni-endzin-analiza/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-pro-zasebni-endzin-analiza — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-pro-zasebni-endzin-analiza/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-pro-zasebni-endzin-analiza', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/spaja-pro-zasebni-endzin-analiza', {
      method: 'POST',
