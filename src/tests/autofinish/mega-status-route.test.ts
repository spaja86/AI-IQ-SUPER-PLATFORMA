// Autofinish — mega-status Route Coverage Test
...
import { GET } from '../../app/api/mega-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 mega-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/mega-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/mega-status', {
