...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1017 Test (#1173)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1017', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1017, 'ciljBroj must be 1017');
  ...
  await test('TOTAL_API_ROUTES je >= 1017', () => {
    assert(TOTAL_API_ROUTES >= 1017, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1017`);
  });

  await test('Poruka sadrži 1017', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1017'), 'poruka mora sadržati 1017');
  });
