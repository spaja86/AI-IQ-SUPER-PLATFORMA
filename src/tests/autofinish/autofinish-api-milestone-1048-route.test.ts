// Autofinish #1309 — API Milestone 1048 Route Coverage Test
// Pokretanje: npx tsx src/tests/autofinish/autofinish-api-milestone-1048-route.test.ts

import fs from 'node:fs';
import path from 'node:path';
import { GET } from '../../app/api/autofinish-api-milestone-1048/route';
import {
  APP_VERSION,
  AUTOFINISH_COUNT,
  TOTAL_API_ROUTES,
...
async function runTests(): Promise<void> {
  console.log('\n🏁 Autofinish API Milestone 1048 — Route Coverage Test Suite (#1309)\n');

  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/autofinish-api-milestone-1048/route.ts');
  ...
  await test('Payload ima osnovna polja', () => {
    assertEqual(body['naziv'] as string, 'Autofinish API Milestone 1048', 'naziv');
  ...
  await test('milestone objekat ima ispravne vrednosti', () => {
    ...
    assertEqual(milestone['ciljBroj'] as number, 1048, 'ciljBroj');
  ...
  await test('poruka je string i pominje 1048', () => {
    const poruka = body['poruka'] as string;
    assert(typeof poruka === 'string' && poruka.length > 0, 'poruka neprazna');
    assert(poruka.includes('1048'), 'poruka sadrzi 1048');
  });

  await test('Konstante su ažurirane', () => {
    assertEqual(APP_VERSION, '57.8.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1309, 'AUTOFINISH_COUNT');
