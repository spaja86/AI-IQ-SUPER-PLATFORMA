// Autofinish — industrija-status Route Coverage Test
...
import { GET } from '../../app/api/industrija-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 industrija-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/industrija-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/industrija-status', {
