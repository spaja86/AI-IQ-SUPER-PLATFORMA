...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1001 Test (#1156)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1001', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1001, 'ciljBroj must be 1001');
  ...
  await test('TOTAL_API_ROUTES je >= 1001', () => {
    assert(TOTAL_API_ROUTES >= 1001, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1001`);
  });

  await test('Poruka sadrži 1001', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1001'), 'poruka mora sadržati 1001');
  });
