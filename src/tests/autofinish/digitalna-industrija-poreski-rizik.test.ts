...
async function runTests(): Promise<void> {
  ...
  await test('Verzije i brojači su ažurirani', () => {
    assertEqual(APP_VERSION, '55.3.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1284, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1150, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1250, 'TOTAL_ROUTES');
  });
