// Autofinish — deploy-pregled Route Coverage Test
...
import { GET } from '../../app/api/deploy-pregled/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 deploy-pregled — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/deploy-pregled/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/deploy-pregled', {
