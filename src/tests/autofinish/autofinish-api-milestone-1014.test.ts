...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1014 Test (#1170)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1014', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1014, 'ciljBroj must be 1014');
  ...
  await test('TOTAL_API_ROUTES je >= 1014', () => {
    assert(TOTAL_API_ROUTES >= 1014, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1014`);
  });

  await test('Poruka sadrži 1014', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1014'), 'poruka mora sadržati 1014');
  });
