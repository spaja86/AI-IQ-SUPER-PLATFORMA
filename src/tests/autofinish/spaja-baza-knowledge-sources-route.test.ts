// Autofinish — spaja-baza-knowledge/sources Route Coverage Test
...
async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-baza-knowledge/sources — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-baza-knowledge/sources/route.ts');
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/spaja-baza-knowledge/sources', {
      method: 'POST',
