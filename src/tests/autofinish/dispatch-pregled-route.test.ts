// Autofinish — dispatch-pregled Route Coverage Test
...
import { GET } from '../../app/api/dispatch-pregled/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 dispatch-pregled — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/dispatch-pregled/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/dispatch-pregled', {
