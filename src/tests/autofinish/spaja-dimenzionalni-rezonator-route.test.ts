// Autofinish — spaja-dimenzionalni-rezonator Route Coverage Test
...
import { GET } from '../../app/api/spaja-dimenzionalni-rezonator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-dimenzionalni-rezonator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-dimenzionalni-rezonator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-dimenzionalni-rezonator', {
