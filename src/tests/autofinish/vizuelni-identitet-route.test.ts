// Autofinish — vizuelni-identitet Route Coverage Test
...
import { GET } from '../../app/api/vizuelni-identitet/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 vizuelni-identitet — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/vizuelni-identitet/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/vizuelni-identitet', {
