// Autofinish — ecosystem-graph Route Coverage Test
...
import { GET } from '../../app/api/ecosystem-graph/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 ecosystem-graph — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/ecosystem-graph/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/ecosystem-graph', {
