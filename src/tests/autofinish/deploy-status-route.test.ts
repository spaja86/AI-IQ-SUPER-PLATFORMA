// Autofinish — deploy-status Route Coverage Test
...
async function runTests(): Promise<void> {
  console.log('\n🏁 deploy-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/deploy-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/deploy-status', {
