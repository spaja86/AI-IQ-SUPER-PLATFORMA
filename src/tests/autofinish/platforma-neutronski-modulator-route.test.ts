// Autofinish — platforma-neutronski-modulator Route Coverage Test
...
import { GET } from '../../app/api/platforma-neutronski-modulator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforma-neutronski-modulator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforma-neutronski-modulator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforma-neutronski-modulator', {
