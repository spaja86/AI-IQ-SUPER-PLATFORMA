// Autofinish — spaja-neutronski-defragmentator Route Coverage Test
...
import { GET } from '../../app/api/spaja-neutronski-defragmentator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-neutronski-defragmentator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-neutronski-defragmentator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-neutronski-defragmentator', {
