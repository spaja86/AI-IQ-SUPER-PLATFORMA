// Autofinish — omega-subatomski-transmiter Route Coverage Test
...
import { GET } from '../../app/api/omega-subatomski-transmiter/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-subatomski-transmiter — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-subatomski-transmiter/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-subatomski-transmiter', {
