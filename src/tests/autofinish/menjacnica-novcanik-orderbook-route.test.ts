// Autofinish — menjacnica-novcanik/orderbook Route Coverage Test
...
import { GET } from '../../app/api/menjacnica-novcanik/orderbook/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica-novcanik/orderbook — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica-novcanik/orderbook/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica-novcanik/orderbook', {
