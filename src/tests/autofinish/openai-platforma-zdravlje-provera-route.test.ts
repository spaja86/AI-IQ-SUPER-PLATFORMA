// Autofinish — openai-platforma-zdravlje-provera Route Coverage Test
...
import { GET } from '../../app/api/openai-platforma-zdravlje-provera/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 openai-platforma-zdravlje-provera — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/openai-platforma-zdravlje-provera/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/openai-platforma-zdravlje-provera', {
