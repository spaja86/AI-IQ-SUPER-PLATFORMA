...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1004 Test (#1160)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1004', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1004, 'ciljBroj must be 1004');
  ...
  await test('TOTAL_API_ROUTES je >= 1004', () => {
    assert(TOTAL_API_ROUTES >= 1004, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1004`);
  });

  await test('Poruka sadrži 1004', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1004'), 'poruka mora sadržati 1004');
  });
