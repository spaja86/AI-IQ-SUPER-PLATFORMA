// Autofinish — autofinish-trigger Route Coverage Test
...
import { GET, POST } from '../../app/api/autofinish-trigger/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 autofinish-trigger — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/autofinish-trigger/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/autofinish-trigger', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/autofinish-trigger', {
      method: 'POST',
