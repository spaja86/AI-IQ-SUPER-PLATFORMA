// Autofinish — evolucija-status Route Coverage Test
...
import { GET } from '../../app/api/evolucija-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 evolucija-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/evolucija-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/evolucija-status', {
