// Autofinish — auth-validacija-pregled Route Coverage Test
...
import { GET } from '../../app/api/auth-validacija-pregled/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 auth-validacija-pregled — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/auth-validacija-pregled/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/auth-validacija-pregled', {
