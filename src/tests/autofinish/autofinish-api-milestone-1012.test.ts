...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1012 Test (#1168)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1012', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1012, 'ciljBroj must be 1012');
  ...
  await test('TOTAL_API_ROUTES je >= 1012', () => {
    assert(TOTAL_API_ROUTES >= 1012, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1012`);
  });

  await test('Poruka sadrži 1012', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1012'), 'poruka mora sadržati 1012');
  });
