// Autofinish — gaming-platforma-dijagnostika Route Coverage Test
...
import { GET } from '../../app/api/gaming-platforma-dijagnostika/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 gaming-platforma-dijagnostika — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/gaming-platforma-dijagnostika/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/gaming-platforma-dijagnostika', {
