// Autofinish — proizvodi Route Coverage Test
...
import { GET } from '../../app/api/proizvodi/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 proizvodi — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/proizvodi/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/proizvodi', {
