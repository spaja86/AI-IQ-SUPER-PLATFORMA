...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1019 Test (#1175)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1019', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1019, 'ciljBroj must be 1019');
  ...
  await test('TOTAL_API_ROUTES je >= 1019', () => {
    assert(TOTAL_API_ROUTES >= 1019, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1019`);
  });

  await test('Poruka sadrži 1019', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1019'), 'poruka mora sadržati 1019');
  });
