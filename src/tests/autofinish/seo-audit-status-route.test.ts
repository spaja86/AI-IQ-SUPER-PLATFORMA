// Autofinish — seo-audit-status Route Coverage Test
...
import { GET } from '../../app/api/seo-audit-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 seo-audit-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/seo-audit-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/seo-audit-status', {
