// Autofinish — oktavni-monolog-status Route Coverage Test
...
import { GET } from '../../app/api/oktavni-monolog-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 oktavni-monolog-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/oktavni-monolog-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/oktavni-monolog-status', {
