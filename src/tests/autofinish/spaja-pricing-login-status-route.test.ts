// Autofinish — spaja-pricing-login-status Route Coverage Test
...
import { GET } from '../../app/api/spaja-pricing-login-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-pricing-login-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-pricing-login-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-pricing-login-status', {
