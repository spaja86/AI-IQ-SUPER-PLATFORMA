// Autofinish — omega-kosmicki-navigator Route Coverage Test
...
import { GET } from '../../app/api/omega-kosmicki-navigator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-kosmicki-navigator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-kosmicki-navigator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-kosmicki-navigator', {
