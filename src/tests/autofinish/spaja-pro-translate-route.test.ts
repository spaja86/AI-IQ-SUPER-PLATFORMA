// Autofinish — spaja-pro/translate Route Coverage Test
...
import { GET, POST } from '../../app/api/spaja-pro/translate/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-pro/translate — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-pro/translate/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-pro/translate', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/spaja-pro/translate', {
      method: 'POST',
