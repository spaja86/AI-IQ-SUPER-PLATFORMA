// Autofinish — menjacnica Route Coverage Test
...
import { GET } from '../../app/api/menjacnica/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica', {
