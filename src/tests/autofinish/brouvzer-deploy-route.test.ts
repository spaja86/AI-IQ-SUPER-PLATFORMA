// Autofinish — brouvzer-deploy Route Coverage Test
...
import { GET, POST } from '../../app/api/brouvzer-deploy/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 brouvzer-deploy — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/brouvzer-deploy/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/brouvzer-deploy', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/brouvzer-deploy', {
      method: 'POST',
