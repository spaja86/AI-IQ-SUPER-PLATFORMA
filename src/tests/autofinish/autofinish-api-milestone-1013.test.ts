...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1013 Test (#1169)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1013', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1013, 'ciljBroj must be 1013');
  ...
  await test('TOTAL_API_ROUTES je >= 1013', () => {
    assert(TOTAL_API_ROUTES >= 1013, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1013`);
  });

  await test('Poruka sadrži 1013', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1013'), 'poruka mora sadržati 1013');
  });
