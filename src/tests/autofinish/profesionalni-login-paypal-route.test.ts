// Autofinish — profesionalni-login-paypal Route Coverage Test
...
import { GET, POST } from '../../app/api/profesionalni-login-paypal/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 profesionalni-login-paypal — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/profesionalni-login-paypal/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/profesionalni-login-paypal', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/profesionalni-login-paypal', {
      method: 'POST',
