...
async function runTests(): Promise<void> {
  ...
  await test('Konstante su ažurirane', () => {
    assertEqual(APP_VERSION, '55.2.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1283, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1149, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1249, 'TOTAL_ROUTES');
  });
