// Autofinish — omega-ai-maksimalni-suport-status Route Coverage Test
...
import { GET } from '../../app/api/omega-ai-maksimalni-suport-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-ai-maksimalni-suport-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-ai-maksimalni-suport-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-ai-maksimalni-suport-status', {
