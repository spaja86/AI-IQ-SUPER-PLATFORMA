...
import { metadata } from '../../app/kristalizacija/page';
import { navigation } from '../../lib/navigation';
import { GET } from '../../app/api/kristalizacija/route';
...
async function runTests(): Promise<void> {
  console.log('\n🏁 kristalizacija - Route Coverage Test Suite\n');

  const entries = sitemap();
  const routeUrl = `${BASE_URL}/kristalizacija`;
  const pagePath = path.resolve(process.cwd(), 'src/app/kristalizacija/page.tsx');
  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/kristalizacija/route.ts');
  const apiRouteSource = fs.readFileSync(apiRoutePath, 'utf8');
  const rezultat = buildKristalizacija('test-user-id');
  ...
  await test('Sitemap sadrži /kristalizacija', () => {
    assert(entries.some((entry) => entry.url === routeUrl), '/kristalizacija nije u sitemap-u');
  });

  await test('metadata.title sadrži Kristalizacija', () => {
    assert(
      typeof metadata.title === 'string' && metadata.title.includes('Kristalizacija'),
      `metadata.title: ${String(metadata.title)}`,
    );
  });

  await test('Navigation sadrži /kristalizacija', () => {
    assert(
      navigation.some((item) => item.href === '/kristalizacija' && item.label === 'Kristalizacija'),
      'navigation nema Kristalizacija link',
    );
  });

  await test('API ruta koristi standard helpere i kristalizacija builder', () => {
    ...
    assert(apiRouteSource.includes('buildKristalizacija'), 'API route ne koristi buildKristalizacija');
  });

  await test('Model rezultata ima očekivana polja', () => {
    assertEqual(rezultat.status, 'aktivan', 'status');
    assert(
      rezultat.indeksKristalizacije >= 0 && rezultat.indeksKristalizacije <= 1,
      'indeksKristalizacije',
    );
    assert(rezultat.stabilnostJezgra >= 0 && rezultat.stabilnostJezgra <= 1, 'stabilnostJezgra');
    assert(rezultat.efikasnostProcesa >= 0 && rezultat.efikasnostProcesa <= 1, 'efikasnostProcesa');
    assert(Array.isArray(rezultat.jezgra) && rezultat.jezgra.length > 0, 'jezgra');
    assert(!Number.isNaN(Date.parse(rezultat.timestamp)), 'timestamp ISO');
  });

  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/kristalizacija', {
      headers: { 'x-forwarded-for': '127.0.1.23' },
    });
