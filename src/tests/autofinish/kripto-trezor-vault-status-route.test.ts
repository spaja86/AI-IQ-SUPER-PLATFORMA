// Autofinish — kripto-trezor/vault-status Route Coverage Test
...
import { GET } from '../../app/api/kripto-trezor/vault-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 kripto-trezor/vault-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/kripto-trezor/vault-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/kripto-trezor/vault-status', {
