// Autofinish — dijagnostika-workflow-orchestracija Route Coverage Test
...
import { GET } from '../../app/api/dijagnostika-workflow-orchestracija/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 dijagnostika-workflow-orchestracija — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/dijagnostika-workflow-orchestracija/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/dijagnostika-workflow-orchestracija', {
