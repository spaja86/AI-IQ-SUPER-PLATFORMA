// Autofinish — omega-temporalni-kristalizator Route Coverage Test
...
import { GET } from '../../app/api/omega-temporalni-kristalizator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-temporalni-kristalizator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-temporalni-kristalizator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-temporalni-kristalizator', {
