// Autofinish — platforms/[platformId]/[...path] Route Coverage Test
...
import { GET, POST } from '../../app/api/platforms/[platformId]/[...path]/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforms/[platformId]/[...path] — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforms/[platformId]/[...path]/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforms/[platformId]/[...path]', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/platforms/[platformId]/[...path]', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: '{',
    });
    const response = await POST(req as unknown as NextRequest, {} as unknown as Parameters<typeof POST>[1]);
