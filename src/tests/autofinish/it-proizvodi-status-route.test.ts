// Autofinish — it-proizvodi-status Route Coverage Test
...
import { GET } from '../../app/api/it-proizvodi-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 it-proizvodi-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/it-proizvodi-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/it-proizvodi-status', {
