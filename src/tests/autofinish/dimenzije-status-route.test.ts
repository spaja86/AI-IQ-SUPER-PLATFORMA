// Autofinish — dimenzije-status Route Coverage Test
...
import { GET } from '../../app/api/dimenzije-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 dimenzije-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/dimenzije-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/dimenzije-status', {
