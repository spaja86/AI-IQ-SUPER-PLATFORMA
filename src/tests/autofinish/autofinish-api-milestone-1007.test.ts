...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1007 Test (#1163)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1007', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1007, 'ciljBroj must be 1007');
  ...
  await test('TOTAL_API_ROUTES je >= 1007', () => {
    assert(TOTAL_API_ROUTES >= 1007, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1007`);
  });

  await test('Poruka sadrži 1007', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1007'), 'poruka mora sadržati 1007');
  });
