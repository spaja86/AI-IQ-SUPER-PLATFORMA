// Autofinish — omega-singularitetni-modulator Route Coverage Test
...
import { GET } from '../../app/api/omega-singularitetni-modulator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-singularitetni-modulator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-singularitetni-modulator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-singularitetni-modulator', {
