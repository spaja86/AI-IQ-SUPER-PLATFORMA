// Autofinish — spaja-baza-knowledge/crawl Route Coverage Test
...
import { GET, POST } from '../../app/api/spaja-baza-knowledge/crawl/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-baza-knowledge/crawl — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-baza-knowledge/crawl/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-baza-knowledge/crawl', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/spaja-baza-knowledge/crawl', {
      method: 'POST',
