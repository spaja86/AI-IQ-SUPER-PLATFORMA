// Autofinish — oktavne-eksponencijalne-funkcije-status Route Coverage Test
...
import { GET } from '../../app/api/oktavne-eksponencijalne-funkcije-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 oktavne-eksponencijalne-funkcije-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/oktavne-eksponencijalne-funkcije-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/oktavne-eksponencijalne-funkcije-status', {
