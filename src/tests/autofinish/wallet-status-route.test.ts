// Autofinish — wallet/status Route Coverage Test
...
import { GET } from '../../app/api/wallet/status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 wallet/status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/wallet/status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/wallet/status', {
