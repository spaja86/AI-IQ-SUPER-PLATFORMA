// Autofinish — autofinish-progress-changelog Route Coverage Test
...
import { GET } from '../../app/api/autofinish-progress-changelog/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 autofinish-progress-changelog — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/autofinish-progress-changelog/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/autofinish-progress-changelog', {
