// Autofinish #1310 — API Milestone 1020 Route Coverage Test
// Pokretanje: npx tsx src/tests/autofinish/autofinish-api-milestone-1020-route.test.ts

import fs from 'node:fs';
import path from 'node:path';
import { GET } from '../../app/api/autofinish-api-milestone-1020/route';
import {
  APP_VERSION,
  AUTOFINISH_COUNT,
  TOTAL_API_ROUTES,
...
async function runTests(): Promise<void> {
  console.log('\n🏁 Autofinish API Milestone 1020 — Route Coverage Test Suite (#1310)\n');

  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/autofinish-api-milestone-1020/route.ts');
  ...
  await test('Payload ima osnovna polja', () => {
    assertEqual(body['naziv'] as string, 'Autofinish API Milestone 1020', 'naziv');
  ...
  await test('milestone objekat ima ispravne vrednosti', () => {
    ...
    assertEqual(milestone['ciljBroj'] as number, 1020, 'ciljBroj');
  ...
  await test('poruka je string i pominje 1020', () => {
    const poruka = body['poruka'] as string;
    assert(typeof poruka === 'string' && poruka.length > 0, 'poruka neprazna');
    assert(poruka.includes('1020'), 'poruka sadrzi 1020');
  });

  await test('Konstante su ažurirane', () => {
    assertEqual(APP_VERSION, '57.9.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1310, 'AUTOFINISH_COUNT');
