// Autofinish — digitalna-observatorija Route Coverage Test
...
import { GET } from '../../app/api/digitalna-observatorija/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 digitalna-observatorija — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/digitalna-observatorija/route.ts');
  ...
  await test('GET smoke provera', async () => {
    ...
    assert(typeof (body as Record<string, unknown>).sistem === 'string', 'sistem mora biti string');
  });
