// Autofinish — omega-subatomski-rezonator Route Coverage Test
...
import { GET } from '../../app/api/omega-subatomski-rezonator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-subatomski-rezonator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-subatomski-rezonator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-subatomski-rezonator', {
