...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1008 Test (#1164)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1008', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1008, 'ciljBroj must be 1008');
  ...
  await test('TOTAL_API_ROUTES je >= 1008', () => {
    assert(TOTAL_API_ROUTES >= 1008, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1008`);
  });

  await test('Poruka sadrži 1008', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1008'), 'poruka mora sadržati 1008');
  });
