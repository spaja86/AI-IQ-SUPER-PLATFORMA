// Autofinish #1425 — API Milestone 1400 Route Coverage Test
// Pokretanje: npx tsx src/tests/autofinish/autofinish-api-milestone-1400-route.test.ts

import fs from 'node:fs';
import path from 'node:path';
import { GET } from '../../app/api/autofinish-api-milestone-1400/route';
import {
  APP_VERSION,
  AUTOFINISH_COUNT,
  TOTAL_API_ROUTES,
...
async function runTests(): Promise<void> {
  console.log('\n🏁 Autofinish API Milestone 1400 — Route Coverage Test Suite (#1425)\n');

  const apiRoutePath = path.resolve(
    process.cwd(),
    'src/app/api/autofinish-api-milestone-1400/route.ts',
  );
  ...
  await test('GET vraća naziv milestone 1400', () => {
    assertEqual(body['naziv'] as string, 'Autofinish API Milestone 1400', 'naziv');
  });
  ...
  await test('GET vraća milestone.ciljBroj === 1400', () => {
    assertEqual(milestone['ciljBroj'] as number, 1400, 'milestone.ciljBroj');
  });
  ...
  await test('Konstante su ažurirane', () => {
    ...
    assert(AUTOFINISH_COUNT >= 1425, 'AUTOFINISH_COUNT baseline');
    assert(TOTAL_API_ROUTES >= 1241, 'TOTAL_API_ROUTES baseline');
    assert(TOTAL_ROUTES >= 1371, 'TOTAL_ROUTES baseline');
  });
