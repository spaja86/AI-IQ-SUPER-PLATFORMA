...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1011 Test (#1167)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1011', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1011, 'ciljBroj must be 1011');
  ...
  await test('TOTAL_API_ROUTES je >= 1011', () => {
    assert(TOTAL_API_ROUTES >= 1011, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1011`);
  });

  await test('Poruka sadrži 1011', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1011'), 'poruka mora sadržati 1011');
  });
