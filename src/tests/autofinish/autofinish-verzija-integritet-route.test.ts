// Autofinish — autofinish-verzija-integritet Route Coverage Test
...
import { GET } from '../../app/api/autofinish-verzija-integritet/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 autofinish-verzija-integritet — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/autofinish-verzija-integritet/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/autofinish-verzija-integritet', {
