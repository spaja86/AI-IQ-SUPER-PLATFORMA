// Autofinish — mobilna-mreza-status Route Coverage Test
...
import { GET } from '../../app/api/mobilna-mreza-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 mobilna-mreza-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/mobilna-mreza-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/mobilna-mreza-status', {
