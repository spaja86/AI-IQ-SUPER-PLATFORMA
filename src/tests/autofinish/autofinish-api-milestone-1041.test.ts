...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1041 Test (#1188)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1041', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1041, 'ciljBroj must be 1041');
  ...
  await test('TOTAL_API_ROUTES je >= 1041', () => {
    assert(TOTAL_API_ROUTES >= 1041, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1041`);
  });

  await test('Poruka sadrži 1041', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1041'), 'poruka mora sadržati 1041');
  });
