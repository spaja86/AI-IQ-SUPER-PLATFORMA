// Autofinish — polimerizacija-2/status Route Coverage Test
...
import { GET } from '../../app/api/polimerizacija-2/status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 polimerizacija-2/status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/polimerizacija-2/status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/polimerizacija-2/status', {
