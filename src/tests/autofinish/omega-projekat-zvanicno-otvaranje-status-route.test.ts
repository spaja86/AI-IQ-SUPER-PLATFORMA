// Autofinish — omega-projekat-zvanicno-otvaranje-status Route Coverage Test
...
import { GET } from '../../app/api/omega-projekat-zvanicno-otvaranje-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-projekat-zvanicno-otvaranje-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-projekat-zvanicno-otvaranje-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-projekat-zvanicno-otvaranje-status', {
