...
import { metadata } from '../../app/rezonancija/page';
import { navigation } from '../../lib/navigation';
import { GET } from '../../app/api/rezonancija/route';
...
async function runTests(): Promise<void> {
  console.log('\n🏁 rezonancija - Route Coverage Test Suite\n');

  const entries = sitemap();
  const routeUrl = `${BASE_URL}/rezonancija`;
  const pagePath = path.resolve(process.cwd(), 'src/app/rezonancija/page.tsx');
  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/rezonancija/route.ts');
  const apiRouteSource = fs.readFileSync(apiRoutePath, 'utf8');
  const rezultat = buildRezonancija('test-user-id');
  ...
  await test('Sitemap sadrži /rezonancija', () => {
    assert(entries.some((entry) => entry.url === routeUrl), '/rezonancija nije u sitemap-u');
  });

  await test('metadata.title sadrži Rezonancija', () => {
    assert(
      typeof metadata.title === 'string' && metadata.title.includes('Rezonancija'),
      `metadata.title: ${String(metadata.title)}`,
    );
  });

  await test('Navigation sadrži /rezonancija', () => {
    assert(
      navigation.some((item) => item.href === '/rezonancija' && item.label === 'Rezonancija'),
      'navigation nema Rezonancija link',
    );
  });

  await test('API ruta koristi standard helpere i rezonancija builder', () => {
    ...
    assert(apiRouteSource.includes('buildRezonancija'), 'API route ne koristi buildRezonancija');
  });

  await test('Model rezultata ima očekivana polja', () => {
    assertEqual(rezultat.status, 'aktivan', 'status');
    assert(
      rezultat.indeksRezonancije >= 0 && rezultat.indeksRezonancije <= 1,
      'indeksRezonancije',
    );
    assert(rezultat.koherentnostMreze >= 0 && rezultat.koherentnostMreze <= 1, 'koherentnostMreze');
    assert(
      rezultat.efikasnostTjuninga >= 0 && rezultat.efikasnostTjuninga <= 1,
      'efikasnostTjuninga',
    );
    assert(Array.isArray(rezultat.cvorovi) && rezultat.cvorovi.length > 0, 'cvorovi');
    assert(!Number.isNaN(Date.parse(rezultat.timestamp)), 'timestamp ISO');
  });

  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/rezonancija', {
      headers: { 'x-forwarded-for': '127.0.1.26' },
    });
