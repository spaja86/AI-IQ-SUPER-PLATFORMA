// Autofinish — wifi-antena-status Route Coverage Test
...
import { GET } from '../../app/api/wifi-antena-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 wifi-antena-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/wifi-antena-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/wifi-antena-status', {
