...
async function runTests(): Promise<void> {
  ...
  await test('Verzije i brojači su ažurirani', () => {
    assertEqual(APP_VERSION, '55.8.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1289, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1155, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1255, 'TOTAL_ROUTES');
  });
