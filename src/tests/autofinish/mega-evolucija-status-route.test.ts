// Autofinish — mega-evolucija-status Route Coverage Test
...
import { GET } from '../../app/api/mega-evolucija-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 mega-evolucija-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/mega-evolucija-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/mega-evolucija-status', {
