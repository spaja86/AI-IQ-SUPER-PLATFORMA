// Autofinish — kripto-trezor/security-check Route Coverage Test
...
import { GET } from '../../app/api/kripto-trezor/security-check/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 kripto-trezor/security-check — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/kripto-trezor/security-check/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/kripto-trezor/security-check', {
