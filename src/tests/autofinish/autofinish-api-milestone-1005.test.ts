...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1005 Test (#1161)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1005', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1005, 'ciljBroj must be 1005');
  ...
  await test('TOTAL_API_ROUTES je >= 1005', () => {
    assert(TOTAL_API_ROUTES >= 1005, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1005`);
  });

  await test('Poruka sadrži 1005', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1005'), 'poruka mora sadržati 1005');
  });
