// Autofinish — sajtovi-status Route Coverage Test
...
import { GET } from '../../app/api/sajtovi-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 sajtovi-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/sajtovi-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/sajtovi-status', {
