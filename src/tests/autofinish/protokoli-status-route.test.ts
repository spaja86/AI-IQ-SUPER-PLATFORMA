// Autofinish — protokoli/status Route Coverage Test
...
import { GET } from '../../app/api/protokoli/status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 protokoli/status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/protokoli/status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/protokoli/status', {
