// Autofinish — billing-engine-matrix Route Coverage Test
...
import { GET } from '../../app/api/billing-engine-matrix/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 billing-engine-matrix — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/billing-engine-matrix/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/billing-engine-matrix', {
