// Autofinish #1416 — API Milestone 1300 Route Coverage Test
// Pokretanje: npx tsx src/tests/autofinish/autofinish-api-milestone-1300-route.test.ts

import fs from 'node:fs';
import path from 'node:path';
import { GET } from '../../app/api/autofinish-api-milestone-1300/route';
import {
  APP_VERSION,
  AUTOFINISH_COUNT,
  TOTAL_API_ROUTES,
...
async function runTests(): Promise<void> {
  console.log('\n🏁 Autofinish API Milestone 1300 — Route Coverage Test Suite (#1416)\n');

  const apiRoutePath = path.resolve(
    process.cwd(),
    'src/app/api/autofinish-api-milestone-1300/route.ts',
  );
  ...
  await test('GET vraća naziv milestone 1300', () => {
    assertEqual(body['naziv'] as string, 'Autofinish API Milestone 1300', 'naziv');
  });
  ...
  await test('GET vraća milestone.ciljBroj === 1300', () => {
    assertEqual(milestone['ciljBroj'] as number, 1300, 'milestone.ciljBroj');
  });
  ...
  await test('Konstante su ažurirane', () => {
    ...
    assert(AUTOFINISH_COUNT >= 1416, 'AUTOFINISH_COUNT baseline');
    assert(TOTAL_API_ROUTES >= 1232, 'TOTAL_API_ROUTES baseline');
    assert(TOTAL_ROUTES >= 1361, 'TOTAL_ROUTES baseline');
  });
