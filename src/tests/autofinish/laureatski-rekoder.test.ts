...
async function runTests(): Promise<void> {
  ...
  await test('AUTOFINISH_COUNT === 1249', () => {
    assertEqual(AUTOFINISH_COUNT, 1249, 'AUTOFINISH_COUNT=1249');
  });
  await test('APP_VERSION === "51.8.0"', () => {
    assertEqual(APP_VERSION, '51.8.0', 'APP_VERSION=51.8.0');
  });
  await test('TOTAL_API_ROUTES === 1118', () => {
    assertEqual(TOTAL_API_ROUTES, 1118, 'TOTAL_API_ROUTES=1118');
  });
  await test('TOTAL_ROUTES === 1197', () => {
    assertEqual(TOTAL_ROUTES, 1197, 'TOTAL_ROUTES=1197');
  });
