// Autofinish — deploy-diagnostics Route Coverage Test
...
import { GET } from '../../app/api/deploy-diagnostics/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 deploy-diagnostics — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/deploy-diagnostics/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/deploy-diagnostics', {
