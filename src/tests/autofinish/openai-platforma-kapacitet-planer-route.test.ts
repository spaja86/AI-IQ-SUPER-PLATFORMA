// Autofinish — openai-platforma-kapacitet-planer Route Coverage Test
...
import { GET } from '../../app/api/openai-platforma-kapacitet-planer/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 openai-platforma-kapacitet-planer — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/openai-platforma-kapacitet-planer/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/openai-platforma-kapacitet-planer', {
