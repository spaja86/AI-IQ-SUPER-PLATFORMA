// Autofinish — autofinish-verzije-diff Route Coverage Test
...
import { GET } from '../../app/api/autofinish-verzije-diff/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 autofinish-verzije-diff — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/autofinish-verzije-diff/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/autofinish-verzije-diff', {
