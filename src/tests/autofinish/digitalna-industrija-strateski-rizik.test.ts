...
async function runTests(): Promise<void> {
  ...
  await test('Verzije i brojači su ažurirani', () => {
    assertEqual(APP_VERSION, '55.1.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1282, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1148, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1248, 'TOTAL_ROUTES');
  });
