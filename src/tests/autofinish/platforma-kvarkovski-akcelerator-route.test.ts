// Autofinish — platforma-kvarkovski-akcelerator Route Coverage Test
...
import { GET } from '../../app/api/platforma-kvarkovski-akcelerator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforma-kvarkovski-akcelerator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforma-kvarkovski-akcelerator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforma-kvarkovski-akcelerator', {
