...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1009 Test (#1165)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1009', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1009, 'ciljBroj must be 1009');
  ...
  await test('TOTAL_API_ROUTES je >= 1009', () => {
    assert(TOTAL_API_ROUTES >= 1009, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1009`);
  });

  await test('Poruka sadrži 1009', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1009'), 'poruka mora sadržati 1009');
  });
