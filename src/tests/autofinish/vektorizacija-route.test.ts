...
import { metadata } from '../../app/vektorizacija/page';
import { navigation } from '../../lib/navigation';
import { GET } from '../../app/api/vektorizacija/route';
...
async function runTests(): Promise<void> {
  console.log('\n🏁 vektorizacija - Route Coverage Test Suite\n');

  const entries = sitemap();
  const routeUrl = `${BASE_URL}/vektorizacija`;
  const pagePath = path.resolve(process.cwd(), 'src/app/vektorizacija/page.tsx');
  const apiRoutePath = path.resolve(process.cwd(), 'src/app/api/vektorizacija/route.ts');
  const apiRouteSource = fs.readFileSync(apiRoutePath, 'utf8');
  const rezultat = buildVektorizacija('test-user-id');
  ...
  await test('Sitemap sadrži /vektorizacija', () => {
    assert(entries.some((entry) => entry.url === routeUrl), '/vektorizacija nije u sitemap-u');
  });

  await test('metadata.title sadrži Vektorizacija', () => {
    assert(
      typeof metadata.title === 'string' && metadata.title.includes('Vektorizacija'),
      `metadata.title: ${String(metadata.title)}`,
    );
  });

  await test('Navigation sadrži /vektorizacija', () => {
    assert(
      navigation.some((item) => item.href === '/vektorizacija' && item.label === 'Vektorizacija'),
      'navigation nema Vektorizacija link',
    );
  });

  await test('API ruta koristi standard helpere i vektorizacija builder', () => {
    ...
    assert(apiRouteSource.includes('buildVektorizacija'), 'API route ne koristi buildVektorizacija');
  });

  await test('Model rezultata ima očekivana polja', () => {
    assertEqual(rezultat.status, 'aktivan', 'status');
    assert(
      rezultat.indeksVektorizacije >= 0 && rezultat.indeksVektorizacije <= 1,
      'indeksVektorizacije',
    );
    assert(rezultat.stabilnostProstora >= 0 && rezultat.stabilnostProstora <= 1, 'stabilnostProstora');
    assert(
      rezultat.efikasnostTransformacije >= 0 && rezultat.efikasnostTransformacije <= 1,
      'efikasnostTransformacije',
    );
    assert(Array.isArray(rezultat.vektori) && rezultat.vektori.length > 0, 'vektori');
    assert(!Number.isNaN(Date.parse(rezultat.timestamp)), 'timestamp ISO');
  });

  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/vektorizacija', {
      headers: { 'x-forwarded-for': '127.0.1.24' },
    });
