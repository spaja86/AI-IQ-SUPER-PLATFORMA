// Autofinish — platforme-status Route Coverage Test
...
import { GET } from '../../app/api/platforme-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforme-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforme-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforme-status', {
