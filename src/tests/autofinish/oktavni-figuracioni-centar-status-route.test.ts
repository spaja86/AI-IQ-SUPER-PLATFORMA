// Autofinish — oktavni-figuracioni-centar-status Route Coverage Test
...
import { GET } from '../../app/api/oktavni-figuracioni-centar-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 oktavni-figuracioni-centar-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/oktavni-figuracioni-centar-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/oktavni-figuracioni-centar-status', {
