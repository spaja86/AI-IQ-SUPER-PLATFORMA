...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1003 Test (#1158)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1003', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1003, 'ciljBroj must be 1003');
  ...
  await test('TOTAL_API_ROUTES je >= 1003', () => {
    assert(TOTAL_API_ROUTES >= 1003, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1003`);
  });

  await test('Poruka sadrži 1003', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1003'), 'poruka mora sadržati 1003');
  });
