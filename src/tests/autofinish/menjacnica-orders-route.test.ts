// Autofinish — menjacnica/orders Route Coverage Test
...
import { GET, POST } from '../../app/api/menjacnica/orders/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica/orders — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica/orders/route.ts');
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/menjacnica/orders', {
      method: 'POST',
