// Autofinish — digitalna-observatorija-pregled Route Coverage Test
...
import { GET } from '../../app/api/digitalna-observatorija-pregled/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 digitalna-observatorija-pregled — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/digitalna-observatorija-pregled/route.ts');
  ...
  await test('GET smoke provera', async () => {
    ...
    assert(isObject(body), 'body mora biti objekt');
    assert(isObject((body as Record<string, unknown>).pregled), 'pregled mora biti objekt');
  });
