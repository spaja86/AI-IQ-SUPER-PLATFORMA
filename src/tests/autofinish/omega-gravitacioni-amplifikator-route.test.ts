// Autofinish — omega-gravitacioni-amplifikator Route Coverage Test
...
import { GET } from '../../app/api/omega-gravitacioni-amplifikator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-gravitacioni-amplifikator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-gravitacioni-amplifikator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-gravitacioni-amplifikator', {
