// Autofinish — dijagnostika-konstante-validacija Route Coverage Test
...
import { GET } from '../../app/api/dijagnostika-konstante-validacija/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 dijagnostika-konstante-validacija — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/dijagnostika-konstante-validacija/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/dijagnostika-konstante-validacija', {
