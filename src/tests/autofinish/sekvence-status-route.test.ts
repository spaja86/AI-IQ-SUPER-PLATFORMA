// Autofinish — sekvence-status Route Coverage Test
...
import { GET } from '../../app/api/sekvence-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 sekvence-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/sekvence-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/sekvence-status', {
