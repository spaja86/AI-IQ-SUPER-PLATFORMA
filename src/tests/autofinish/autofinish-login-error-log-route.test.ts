// Autofinish — autofinish-login-error-log Route Coverage Test
...
import { GET } from '../../app/api/autofinish-login-error-log/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 autofinish-login-error-log — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/autofinish-login-error-log/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/autofinish-login-error-log', {
