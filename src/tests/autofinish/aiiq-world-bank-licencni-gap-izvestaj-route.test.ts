// Autofinish — aiiq-world-bank-licencni-gap-izvestaj Route Coverage Test
...
async function runTests(): Promise<void> {
  console.log('\n🏁 aiiq-world-bank-licencni-gap-izvestaj — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/aiiq-world-bank-licencni-gap-izvestaj/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/aiiq-world-bank-licencni-gap-izvestaj', {
