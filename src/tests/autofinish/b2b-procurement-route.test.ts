// Autofinish — b2b-procurement Route Coverage Test
...
import { GET, POST } from '../../app/api/b2b-procurement/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 b2b-procurement — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/b2b-procurement/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/b2b-procurement', {
  ...
  await test('POST odbija nevalidan JSON payload', async () => {
    const req = new Request('http://localhost/api/b2b-procurement', {
      method: 'POST',
