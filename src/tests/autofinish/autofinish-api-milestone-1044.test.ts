...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1044 Test (#1191)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1044', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1044, 'ciljBroj must be 1044');
  ...
  await test('TOTAL_API_ROUTES je >= 1044', () => {
    assert(TOTAL_API_ROUTES >= 1044, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1044`);
  });

  await test('Poruka sadrži 1044', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1044'), 'poruka mora sadržati 1044');
  });
