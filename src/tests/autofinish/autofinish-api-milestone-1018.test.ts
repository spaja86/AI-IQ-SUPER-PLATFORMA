...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1018 Test (#1174)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1018', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1018, 'ciljBroj must be 1018');
  ...
  await test('TOTAL_API_ROUTES je >= 1018', () => {
    assert(TOTAL_API_ROUTES >= 1018, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1018`);
  });

  await test('Poruka sadrži 1018', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1018'), 'poruka mora sadržati 1018');
  });
