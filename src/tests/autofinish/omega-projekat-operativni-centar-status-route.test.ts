// Autofinish — omega-projekat-operativni-centar-status Route Coverage Test
...
import { GET } from '../../app/api/omega-projekat-operativni-centar-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-projekat-operativni-centar-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-projekat-operativni-centar-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-projekat-operativni-centar-status', {
