// Autofinish — spaja-neutronski-sintetizator Route Coverage Test
...
import { GET } from '../../app/api/spaja-neutronski-sintetizator/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-neutronski-sintetizator — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-neutronski-sintetizator/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-neutronski-sintetizator', {
