// Autofinish — vlasnicki-vip-plan Route Coverage Test
...
import { GET } from '../../app/api/vlasnicki-vip-plan/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 vlasnicki-vip-plan — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/vlasnicki-vip-plan/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/vlasnicki-vip-plan', {
