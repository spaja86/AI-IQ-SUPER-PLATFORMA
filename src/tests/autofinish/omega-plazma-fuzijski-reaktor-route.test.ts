// Autofinish — omega-plazma-fuzijski-reaktor Route Coverage Test
...
import { GET } from '../../app/api/omega-plazma-fuzijski-reaktor/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 omega-plazma-fuzijski-reaktor — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/omega-plazma-fuzijski-reaktor/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/omega-plazma-fuzijski-reaktor', {
