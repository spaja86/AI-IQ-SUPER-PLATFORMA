...
async function runTests(): Promise<void> {
  ...
  await test('TOTAL_API_ROUTES === 927', () => {
    assertEqual(TOTAL_API_ROUTES, 927, 'TOTAL_API_ROUTES=927');
  });

  await test('getLastNIterations(10) zadnja = #880', () => {
    const r = getLastNIterations(10);
    assertEqual(r[r.length - 1].broj, 880, 'zadnja=#880');
  });
