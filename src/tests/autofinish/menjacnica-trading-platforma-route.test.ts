// Autofinish — menjacnica-trading-platforma Route Coverage Test
...
import { GET } from '../../app/api/menjacnica-trading-platforma/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica-trading-platforma — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica-trading-platforma/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica-trading-platforma', {
