// Autofinish — proksi-status Route Coverage Test
...
import { GET } from '../../app/api/proksi-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 proksi-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/proksi-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/proksi-status', {
