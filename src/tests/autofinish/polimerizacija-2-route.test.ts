...
async function runTests(): Promise<void> {
  ...
  await test('Konstante su ažurirane', () => {
    assertEqual(APP_VERSION, '59.82.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1430, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1251, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1387, 'TOTAL_ROUTES');
    assertEqual(TOTAL_PAGES, 136, 'TOTAL_PAGES');
  });
