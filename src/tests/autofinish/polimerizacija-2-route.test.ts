...
async function runTests(): Promise<void> {
  ...
  await test('Konstante su ažurirane', () => {
    assertEqual(APP_VERSION, '59.83.0', 'APP_VERSION');
    assertEqual(AUTOFINISH_COUNT, 1431, 'AUTOFINISH_COUNT');
    assertEqual(TOTAL_API_ROUTES, 1252, 'TOTAL_API_ROUTES');
    assertEqual(TOTAL_ROUTES, 1389, 'TOTAL_ROUTES');
    assertEqual(TOTAL_PAGES, 137, 'TOTAL_PAGES');
  });
