// Autofinish — menjacnica-usluge-planovi Route Coverage Test
...
import { GET } from '../../app/api/menjacnica-usluge-planovi/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica-usluge-planovi — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica-usluge-planovi/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica-usluge-planovi', {
