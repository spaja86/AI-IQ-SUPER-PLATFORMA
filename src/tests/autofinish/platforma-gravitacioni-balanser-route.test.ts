// Autofinish — platforma-gravitacioni-balanser Route Coverage Test
...
import { GET } from '../../app/api/platforma-gravitacioni-balanser/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 platforma-gravitacioni-balanser — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/platforma-gravitacioni-balanser/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/platforma-gravitacioni-balanser', {
