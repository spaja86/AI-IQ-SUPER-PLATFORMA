...
async function run(): Promise<void> {
  console.log('\n📋 Autofinish API Milestone 1002 Test (#1157)\n');
  ...
  await test('Payload ima osnovna polja', async () => {
    ...
    assert(body.naziv === 'Autofinish API Milestone 1002', 'naziv mismatch');
  ...
  await test('Milestone blok ima očekivane vrednosti', async () => {
    ...
    assert(Number(milestone.ciljBroj) === 1002, 'ciljBroj must be 1002');
  ...
  await test('TOTAL_API_ROUTES je >= 1002', () => {
    assert(TOTAL_API_ROUTES >= 1002, `TOTAL_API_ROUTES=${TOTAL_API_ROUTES} < 1002`);
  });

  await test('Poruka sadrži 1002', async () => {
    const res = await GET();
    const body = (await res.json()) as Record<string, unknown>;
    const poruka = String(body.poruka ?? '');
    assert(poruka.includes('1002'), 'poruka mora sadržati 1002');
  });
