// Autofinish — kripto-trezor/diversification Route Coverage Test
...
import { GET } from '../../app/api/kripto-trezor/diversification/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 kripto-trezor/diversification — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/kripto-trezor/diversification/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/kripto-trezor/diversification', {
