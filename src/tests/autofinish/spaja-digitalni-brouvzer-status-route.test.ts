// Autofinish — spaja-digitalni-brouvzer-status Route Coverage Test
...
import { GET } from '../../app/api/spaja-digitalni-brouvzer-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 spaja-digitalni-brouvzer-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/spaja-digitalni-brouvzer-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/spaja-digitalni-brouvzer-status', {
