// Autofinish — menjacnica-novcanik/settlement-status Route Coverage Test
...
import { GET } from '../../app/api/menjacnica-novcanik/settlement-status/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 menjacnica-novcanik/settlement-status — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/menjacnica-novcanik/settlement-status/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/menjacnica-novcanik/settlement-status', {
