// Autofinish — issuer-licensing/expirations Route Coverage Test
...
import { GET } from '../../app/api/issuer-licensing/expirations/route';

async function runTests(): Promise<void> {
  console.log('\n🏁 issuer-licensing/expirations — Route Coverage Test Suite\n');

  const routePath = path.resolve(process.cwd(), 'src/app/api/issuer-licensing/expirations/route.ts');
  ...
  await test('GET smoke provera', async () => {
    const request = new Request('http://localhost/api/issuer-licensing/expirations', {
