// EKSPONAT GLAVNOG JEZGRA — API ruta
// Kompanija SPAJA — Digitalna Industrija
//
// GET /api/eksponat-glavnog-jezgra
...
export async function GET(request: NextRequest) {
  try {
    ...
    const allowed = await checkRateLimitGlobal(
      rateLimitKey(ip, '/api/eksponat-glavnog-jezgra'),
      30,
      60,
    );
    if (!allowed) {
    ...
    const rezultat = buildEksponatGlavnogJezgra(user.id);
    return apiSuccess({ rezultat });
  } catch (error) {
    return apiInternalError('eksponat-glavnog-jezgra', error);
  }
}
