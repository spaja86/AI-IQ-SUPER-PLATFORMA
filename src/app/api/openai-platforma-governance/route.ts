...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    governance: {
      ...
      enterpriseKomercijala: {
        posiljalac: enterpriseZahtevi[0]?.posiljalac ?? 'sales@spaja.rs',
        readyToSend: enterpriseZahtevi.filter((paket) => paket.status !== 'u_pripremi').length,
        kanali: enterpriseZahtevi.map((paket) => ({
          provajder: paket.provajder,
          status: paket.status,
          url: paket.kanalPodnosenja.url,
        })),
        governanceSpremnost: operativa.spremnost.enterprise,
      },
    },
    ...
    dijagnostike: [
      ...
      { id: 'openai-gov-006', naziv: 'Enterprise request paketi', status: enterpriseZahtevi.every((paket) => paket.status !== 'u_pripremi') ? 'ok' : 'warning', opis: 'Vercel, GitHub i OpenAI enterprise paketi pripremljeni preko kompanijskih mejlova i zvaničnih contact form kanala (OpenAI: https://openai.com/contact-sales)' },
    ],
