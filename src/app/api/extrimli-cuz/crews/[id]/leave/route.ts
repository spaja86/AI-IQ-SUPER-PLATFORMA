// SpajaUltraOmegaCore -∞Ω+∞ — EXTRIMLI CUZ API: /api/extrimli-cuz/crews/[id]/leave
// Kompanija SPAJA — Digitalna Industrija

import type { NextRequest } from 'next/server';
import { apiError, apiInternalError, apiSuccess } from '@/lib/api/response';
import { leaveCrew, CUZ_CONTRACT_VERSION, CUZ_MODULE_VERSION } from '@/lib/extrimli-cuz';
...
export async function POST(
  ...
  try {
    ...
    const result = leaveCrew(params.id, b.athleteId);
    const response = apiSuccess(result, result.success ? 200 : 422);
    setHeaders(response);
    return response;
  } catch (error) {
    return apiInternalError('extrimli-cuz/crews/[id]/leave', error);
  }
}
