...
function getAutofinishOpis(n: number): string {
  const opisi: Record<number, string> = {
    ...
    1415: `Autofinish #1415 — PENTRACIJA modul: src/lib/pentracija.ts, PentestFinding/PentestReport/PentestKategorija (OWASP Top 10), 4 API rute (/api/pentracija, /status, /nalazi, /sken), UI dashboard, testovi pentracija-lib + pentracija-route, TOTAL_API_ROUTES 1226→1230, TOTAL_ROUTES 1355→1359, TOTAL_PAGES 129→130, AUTOFINISH_COUNT 1414→1415, APP_VERSION 59.71.0→59.72.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1416: `Autofinish #1416 — API Milestone 1300 route + coverage test, TOTAL_API_ROUTES korekcija 1230→1232, TOTAL_ROUTES 1359→1361, AUTOFINISH_COUNT 1415→1416, APP_VERSION 59.72.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1417: `Autofinish #1417 — API Milestone 1350 route + coverage test, TOTAL_API_ROUTES 1232→1233, TOTAL_ROUTES 1361→1362, AUTOFINISH_COUNT 1416→1417, APP_VERSION 59.72.0→59.73.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1418: `Autofinish #1418 — PANETRACIJA 2 faza 1: lib src/lib/panetracija-2.ts — PentestFindingV2 (CVSS 3.1 vector, CWE ID, prioritet), PentestScanSession, PentestReportV2 (history + trendovi), 20 proširenih OWASP+supply-chain nalaza, AUTOFINISH_COUNT 1417→1418, APP_VERSION 59.73.0→59.74.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1419: `Autofinish #1419 — PANETRACIJA 2 faza 2: 6 API ruta (/api/panetracija-2, /status, /nalazi, /sken, /istorija, /trendovi), ring-buffer scan istorija, TOTAL_API_ROUTES 1233→1239, AUTOFINISH_COUNT 1418→1419, APP_VERSION 59.74.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1420: `Autofinish #1420 — PANETRACIJA 2 faza 3: UI dashboard src/app/panetracija-2/page.tsx (score, trend sparkline, OWASP+CWE grid, filter tabela, scan istorija), PentestFilterTable.tsx, TOTAL_ROUTES 1362→1369, TOTAL_PAGES 130→131, AUTOFINISH_COUNT 1419→1420, APP_VERSION 59.74.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1421: `Autofinish #1421 — PANETRACIJA 2 finalizacija: testovi panetracija-2-lib + 6 route testova, ΩPermissionMatrix panetracija2:read/execute, AUTOFINISH_COUNT 1420→1421, APP_VERSION 59.74.0→59.75.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1422: `Autofinish #1422 — Fix panetracija-2/nalazi: dodat 'auth' u VALID_KATEGORIJE kao alias za broken-auth filter, AUTOFINISH_COUNT 1421→1422, APP_VERSION 59.75.0→59.75.1, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    ...
    1427: `Autofinish #1427 — KRISTALIZACIJA modul: src/lib/kristalizacija.ts, GET /api/kristalizacija, UI /kristalizacija, sekvenca kristalizacija-page.ts, route coverage test, TOTAL_API_ROUTES 1242→1243, TOTAL_ROUTES 1373→1375, TOTAL_PAGES 132→133, AUTOFINISH_COUNT 1426→1427, APP_VERSION 59.78.0→59.79.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1428: `Autofinish #1428 — POLIMERIZACIJA 2 modul: src/lib/polimerizacija-2.ts, 6 API ruta (/api/polimerizacija-2 + /status /lanci /sken /istorija /trendovi), UI /polimerizacija-2 i PolimerizacijaLanciTable, sekvenca polimerizacija-2-page.ts, lib+route coverage testovi, TOTAL_API_ROUTES 1243→1249, TOTAL_ROUTES 1375→1383, TOTAL_PAGES 133→134, AUTOFINISH_COUNT 1427→1428, APP_VERSION 59.79.0→59.80.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1429: `Autofinish #1429 — VEKTORIZACIJA modul: src/lib/vektorizacija.ts, GET /api/vektorizacija, UI /vektorizacija, sekvenca vektorizacija-page.ts, route coverage test, TOTAL_API_ROUTES 1249→1250, TOTAL_ROUTES 1383→1385, TOTAL_PAGES 134→135, AUTOFINISH_COUNT 1428→1429, APP_VERSION 59.80.0→59.81.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1430: `Autofinish #1430 — SINTETIZACIJA modul: src/lib/sintetizacija.ts, GET /api/sintetizacija, UI /sintetizacija, sekvenca sintetizacija-page.ts, route coverage test, TOTAL_API_ROUTES 1250→1251, TOTAL_ROUTES 1385→1387, TOTAL_PAGES 135→136, AUTOFINISH_COUNT 1429→1430, APP_VERSION 59.81.0→59.82.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1431: `Autofinish #1431 — REZONANCIJA modul: src/lib/rezonancija.ts, GET /api/rezonancija, UI /rezonancija, sekvenca rezonancija-page.ts, route coverage test, TOTAL_API_ROUTES 1251→1252, TOTAL_ROUTES 1387→1389, TOTAL_PAGES 136→137, AUTOFINISH_COUNT 1430→1431, APP_VERSION 59.82.0→59.83.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    ...
    1438: `Autofinish #1438 — Dedicated route coverage test za /api/polimerizacija-2/status (GET smoke + response helper + konstante), TOTAL_API_ROUTES 1254→1255, TOTAL_ROUTES 1393→1394, TOTAL_PAGES 139→139, AUTOFINISH_COUNT 1437→1438, APP_VERSION 59.89.0→59.90.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
    1439: `Autofinish #1439 — OpenAI Partnership + Ownership Transition: src/lib/kompanija-epilog.ts (kanonski versioned epilog), src/lib/openai-partnership-lifecycle.ts (8-stanje lifecycle + evidence pack + post-acceptance plan), src/lib/ownership-transition-runbook.ts (4-fazni ownership runbook sa go/no-go), /api/openai-partnership-lifecycle (GET+POST), /api/ownership-transition-runbook (GET), TOTAL_API_ROUTES 1255→1257, TOTAL_ROUTES 1394→1396, AUTOFINISH_COUNT 1438→1439, APP_VERSION 59.90.0→59.91.0, ${TOTAL_ROUTES} ruta, ${TOTAL_API_ROUTES} API, ${TOTAL_DIAGNOSTIKA} dijagnostike`,
  };
